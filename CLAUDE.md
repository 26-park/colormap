...
- **⭐ 다음 세션 시작점**: P0/P1 전부 완료, P2도 대부분 완료 — 남은 건 아래 순서대로.
  1. 게시물 공개범위 사후 변경 (현재는 작성 시점에만 지정 가능 — 게시물 상세 또는 별도 화면에 수정 UI 필요)
  2. 계정 공개범위(public/private) 토글 UI (`profiles`엔 컬럼 있으나 설정 화면에 UI 없음, `app/settings.tsx`)
  3. (우선순위 낮음, 급하지 않음) `expo-modules-core` 직접 의존성 정리 — `expo` 패키지가 재노출하는 API로 교체
  4. **출시 준비**: 구글 플레이 콘솔 개발자 등록($25) + AAB 빌드(EAS, `preview` 프로필은 APK라 별도 프로필 필요) + 스토어 등록 자료(스크린샷, 설명, 개인정보처리방침 URL) 준비
