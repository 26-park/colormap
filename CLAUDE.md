...
- **⭐ 다음 세션 시작점 (2026-08-31)**: 로드맵 5번 **Phase T의 "남은 목록"** 부터 — **[T-1] 프로필 글꼴 배율**, **[T-2] GPS 없이 수동 핀**, 그리고 설계 변경이라 미뤄둔 **4순위(줌아웃 시군구 색칠)·5순위(시군구 FAB)**. 전체 순서는 아래 로드맵대로 (배경은 "⭐ 출시 시점 방향 결정" 참고).
  ...
  5. ⭐⭐ **진행 중: 출시 전 실기기 다듬기 (Phase T) — 2026-08-23 추가**. 갤럭시 실기기에 최신 빌드를 올려 **실제로 써보면서 다듬는다.** 새 기능을 넓히는 단계가 아니라, 에뮬에서 안 보이던 것(터치 타깃, 실제 GPS, 실제 사진, 스크롤 감, 성능, 권한 다이얼로그)을 잡는 단계다.
     ...
     - **⭐⭐ Phase T 남은 목록 (2026-08-31 3차 실기기 검증 후 확정 — 다음 세션은 여기서 시작)**:
       ...
       - **⛔ [T-1] 프로필 화면만 글꼴 배율에서 계속 깨진다 (실기기 갤럭시 최대 배율)**: 상한 1.2 + `paddingRight: 8`을 적용했는데도 **프로필만** 실패한다(다른 화면은 정상).
         - ⚠️⚠️ **에뮬 판정이 또 틀렸다** — `font_scale 2.0` + `wm density 480`(폭 360dp)에서 "전 화면 정상"으로 판정했는데 실기기에서 실패했다. **"에뮬레이터로 검증할 수 없는 것들" 섹션의 판정 규칙이 글꼴 배율에도 그대로 적용된다 — 에뮬 통과를 해결 근거로 삼지 말 것.**
         - **다음 세션 조사 순서**: ① **갤럭시의 실제 값부터 읽을 것** — `adb shell settings get system font_scale` (+ `wm density`). 에뮬 2.0보다 큰지, 화면 크기 설정이 다르게 작용하는지가 갈린다. ② **왜 프로필만인지** — 다른 화면과 구조가 다른 지점을 의심할 것(나라 칩 가로 스크롤 `ScrollView`, 통계 카드 3분할 `flex:1`+`statDivider` 고정 폭 1dp, 아바타 아래 중앙정렬 텍스트).
         - ⚠️ 상한을 더 낮추는 것으로 때우지 말 것 — 이미 1.3→1.2로 한 번 밀어냈고, **근본 원인은 Pretendard 측정 폭**이다(위 "배율 절벽" 항목). 실제 값을 읽기 전에 숫자부터 낮추면 접근성만 깎고 또 빗나간다.
       - **⛔ [T-2] GPS 없이 수동으로 핀을 찍을 수 있어야 한다**: 지도 화면에서 "현재 위치"를 눌러도 **핀이 안 찍히는 경우가 있다**(실기기).
         - **방향**: 작성 화면 미니맵은 이미 탭·드래그로 위치 지정이 되므로(2순위에서 검증됨), **GPS 실패 시에도 그 경로를 쓸 수 있게** 여는 쪽으로 간다. GPS를 고치는 게 아니라 GPS 없이도 진행 가능하게 만드는 것이 요구사항이다.
         - **확인할 것**: ① 6순위에서 `getLastKnownPositionAsync`를 먼저 쓰도록 바꿨는데 **캐시가 없거나 `maxAge` 2분을 넘긴 경우 어떻게 되는지** ② `canPost`에 걸어둔 `!gpsLoading`이 이 상황에서 게시를 영구히 막지는 않는지.
...
- ⭐ **판정 규칙**: **터치 회전 / 폰트 측정 / 시스템 글꼴 배율 / 실제 GPS / 제조사 키보드 / 멀티터치 제스처 충돌**이 얽힌 항목은 **에뮬 통과를 해결 근거로 삼지 말 것.** 에뮬은 "안 고쳐졌다"는 반증에는 쓸 수 있어도 "고쳐졌다"는 증명에는 못 쓴다.
  - ⚠️⚠️ **이 규칙을 알고도 또 어겼다 (2026-08-31, Phase T 6순위)**: 글꼴 배율을 `font_scale 2.0` + `wm density 480`까지 올려 "전 화면 정상"으로 판정했는데 **실기기 갤럭시 최대 배율에서 프로필이 그대로 깨졌다**(T-1). 에뮬에서 조건을 아무리 극단으로 밀어도 **제조사 설정이 실제로 어떤 값을 만드는지는 기기에서 읽어야 안다** — 다음엔 **`adb shell settings get system font_scale`로 실기기 값을 먼저 읽고** 그 값으로 재현할 것.
