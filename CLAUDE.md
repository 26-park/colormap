...
- **⭐ 다음 세션 시작점**: 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  ...
  2. ✅ **좋아요 — 완료 (Phase P)**
  3. ✅ **댓글 — 완료 (Phase Q)**
  4. ⭐ **진행 중: 한국 시군구 색칠 (Phase S)** — S-0(**2026-08-16 OSM/ODbL로 소스 재확정**)·S-1(city 잔재 정리) 완료. **S-2 진행 중, 아래 순서대로:**
     1. **Overpass로 `admin_level=6` 229개 추출**(시도별 17분할) — 504가 반복되면 **Geofabrik pbf(272MB) 받아 로컬 처리로 전환**. ⚠️ **일회성 작업이니 재시도 파이프라인을 정교하게 만들지 말 것.**
     2. **⭐ 세종시 병합 (필수)** — 세종은 `admin_level=6`에 없고 `admin_level=4`에 있다. **안 하면 지도에 구멍이 생긴다.**
     3. **코드 보정** — `ref:KR:mois:admin` 결측 9개(울산 5 = 알려진 값 / 인천 신설 4 = MOIS 고시 확인). **없으면 결측 허용하되 surrogate id는 발급**해 나중에 채울 수 있게.
     4. **개수·속성 검증** — ⚠️ **229라는 숫자 일치를 검증으로 삼지 말 것**(아래 Phase S의 "229 함정"). 시/군/구 내역과 세종 유무로 대조.
     5. **단순화 실험 + 용량 측정** — APK 번들 크기 직결(현행 `countries.json`이 284KB/242피처인 게 비교 기준).
  5. 장소검색(지오코딩)
...
- **완료 (C-1 ~ Phase D)**:
  ...
  - Phase S: **한국 시군구 색칠 (조사 완료 2026-07-31, 진행 중)**. 세계는 나라 단위 유지, **한국만 시군구 단위로 세분**. 지도 색칠은 진입점이라는 제품 정의는 그대로.
    ...
    - **단계 분할**: **S-0 라이선스 확정 ✅ 완료**(2026-08-16 OSM/ODbL로 소스 재확정) → **S-1 city 잔재 정리 ✅ 완료**(커밋 `1f17991`) → **S-2 진행 중**(추출 → **세종 병합(필수)** → 코드 보정 → 개수·속성 검증 → 단순화 실험/용량 측정 → 경계 테이블+적재) → S-3(`posts.sgg_id`+판정 트리거+backfill) → S-4(방문 테이블+RLS) → S-5(지도 렌더링 — **해안선 클립 → 재단순화 → 용량 재측정**이 여기 포함) → S-6(시군구 상세/색 선택). **출처표시 UI("지도 데이터 출처")는 S-5 또는 S-6 범위에 포함**(아래 라이선스 원칙 참고).
