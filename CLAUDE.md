...
- 작성: 위치는 **지도 핀 + 장소 검색(지오코딩) 둘 다** 필수 — 나라(country_code)는 핀 좌표 역지오코딩으로 자동 파생(C-2-2b), 사용자 입력 불필요. **도시 선택은 v1.1로 연기**(cities 데이터가 채워진 뒤 부활, 그때까지 `posts.city_id`는 nullable — C-2-2a) · 사진 **다중 첨부 + 대표 지정**(post_media 1:N, 대표=order_index 0)
...
- **도시**: v1.1로 연기. 사전 정의된 참조 목록(`cities` 테이블)에서 선택하는 방식 자체는 유지, cities 데이터가 채워질 때까지 `posts.city_id`는 nullable(C-2-2a).
