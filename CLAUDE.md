...
- 세계지도(choropleth, 평면↔3D 토글)에서 가본 **도시 색칠** (도시 단위, 도시별 색 선택)
- 도시 누르면 → 도시 상세(접힌 지도 미리보기 + 인스타식 정사각형 사진 그리드) → **위치별 사진+글 기록** (자유 핀, 위경도)
- 작성: 위치는 **지도 핀 + 장소 검색(지오코딩) 둘 다**, 사진 **다중 첨부 + 대표 지정**(post_media 1:N, 대표=order_index 0)
...
- **공통 컴포넌트**: 인스타식 정사각형 사진 그리드 — 도시 상세·프로필이 재사용.
- **도시 상세**: 상단 접힌 지도 미리보기 + 사진 그리드, 이름 옆 ▾로 도시 전환, 도시 색 선택은 더보기(…) 안.
- ⚠️ **열린 질문**: 세계지도 도시 색칠(choropleth)은 도시 단위인데 **도시 경계 폴리곤 데이터 소스 미정** → 지도 구현 단계에서 결정 (PRD 10장).
