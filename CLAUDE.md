...
- **⭐ 다음 세션 시작점**: P0/P1 전부 완료, P2도 대부분 완료 — 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  1. P2 마무리: 게시물 공개범위 사후 변경(현재는 작성 시점에만 지정 가능) + 계정 공개범위(public/private) 토글 UI(`profiles`엔 컬럼 있으나 설정 화면에 UI 없음, `app/settings.tsx`)
  2. (우선순위 낮음, 급하지 않음) `expo-modules-core` 직접 의존성 정리 — `expo` 패키지가 재노출하는 API로 교체
  3. 친구 기능 (상호 수락)
  4. 좋아요
  5. 댓글
  6. 장소검색(지오코딩) / 3D 지구본 토글
