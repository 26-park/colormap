...
- **🎉🎉 한국 시군구 색칠(Phase S) 전체 완료 (2026-08-18)**: S-0 라이선스 확정(OSM/ODbL — 정부 포털 2곳은 **변경금지·절차**로 탈락) / S-1 city 잔재 정리 / S-2 경계 230개 추출·검증 / S-3 서버측 판정 트리거 / S-4 방문 테이블+RLS+동기화 트리거 / S-5 해안선 클립·지도 렌더·색칠 바인딩·출처표시 / **S-6 시군구 상세(진입·그리드·색 선택)**. 상세는 아래 Phase S 참고.
- **⭐ ODbL 충족 상태 (2026-08-17 기준)**: **4.2(고지)·4.6(파생 DB 접근)** = public repo + **앱 내 가공본 링크**(`설정 → 지도 데이터 출처 → 가공본 내려받기`) / **4.3(Produced Work 고지)** = `app/map-credits.tsx` 화면. **⛔ repo를 private으로 전환하면 4.6 즉시 위반**이고 앱 안의 링크도 죽는다.
- **⭐⭐ 다음 세션 시작점 (2026-09-11)**: **친구 프로필 / 친구 지도 보기**. Phase T의 실기기 다듬기 항목은 **전부 끝났다**([T-1] 글꼴 배율 · [T-2] GPS 수동 핀 · 4순위 줌아웃 색칠 · 5순위 시군구 FAB · 작성 화면 핀 표시 · "모두" 탭 숨김 · 시군구 오탭 · 지도 핀 표시 — 2026-09-10 전부 완료, 5차 빌드 `56437df`).
  - **⭐ 이 작업이 `sgg_visits`/`country_visits` SELECT 정책이 **처음 깨어나는 지점**이다** — Phase N/S에 "남의 지도를 보는 화면이 생길 때 깨어난다"고 적어둔 그 잠자는 방어다. 그리고 **"모두" 탭을 숨기면서 사라진 `can_view_post` 화면 검증 지점이 여기서 되살아난다**(위 "권한/가시성 모델" 참고) — **2계정 대조를 반드시 넣을 것.**
  - **조사할 것**: ① 화면 구조(내 프로필/지도를 재사용할지, `app/user/[id].tsx` 같은 별도 화면을 만들지) ② 친구 프로필에 보이는 것(통계·나라 칩·그리드)이 각각 RLS로 걸러지는지 아니면 추가 필터가 필요한지 ③ 친구 지도(색칠은 친구 것, 핀도 친구 것 — 핀 기능과 얽힘) ④ 진입 경로(친구 목록 카드 / 검색 결과 / 댓글 작성자?) ⑤ **비친구·비공개 계정 처리** — Phase L에서 "계정 자체는 검색되지만 내역은 친구만"으로 설계했다.
  - ⚠️ **조사 표면이 넓은 작업이다** — 신선한 세션에서 다루기로 한 이유다.
  - 이후 순서는 아래 로드맵대로 (배경은 "⭐ 출시 시점 방향 결정" 참고).
  1. ✅ **친구 기능 UI — 완료 (Phase O)**
  2. ✅ **좋아요 — 완료 (Phase P)**
  3. ✅ **댓글 — 완료 (Phase Q)**
  4. ✅ **한국 시군구 색칠 (Phase S) — 완료 (2026-08-16~18)**. S-0~S-6 전부. 상세는 아래 Phase S 참고.
     - ⚠️ **미결 — 인천 신설 4개 코드**: `sgg_code` null로 남겨뒀다. surrogate id 설계라 아무것도 막지 않는다(색칠 바인딩도 `osm_id` 기준, 시도 라벨은 `lib/sidoNamesKo.ts`가 osm_id로 예외 매핑). MOIS 고시가 나오면 채울 것 — **추측한 코드를 넣지 말 것.**
     - ⏸️ **미확인 보류 (무해)**: **최대줌 16에서 `+` 비활성** — 최소줌에서 같은 로직을 확인했다.
  ...
  5. 🎉🎉 **완료: 출시 전 실기기 다듬기 (Phase T) — 2026-08-23 시작 → 2026-09-11 종료. 실기기 검증 5사이클.**
     - **⭐ 5차(`56437df`)에서 판정된 것 — 전부 통과**: **작성 화면 핀 보이기**(circle 레이어 전환으로 해결, 4차 최대 이슈) / **지도 탭 내 글 핀 표시 + 설정 토글** / **시군구 오탭 정밀도** / **"모두" 탭 숨김** / **4차 잔여 4건**(캐시 핀 정정 대기 · 군도 bbox · 시군구 경계선 가시성 · 색칠 왕복).
     - **⭐⭐ 사용자 판정으로 닫은 두 가지 — 추가 작업 불필요**:
       - **드래그 없이 탭 이동만으로 충분하다.** → **`circle` + (안 보이는) `ViewAnnotation` 조합은 만들지 않는다.** 근거는 남겨둔다: 200dp 미니맵에서는 탭이 오히려 빠르고, ViewAnnotation 을 되살리면 레이어 순서에 묻히는 문제를 다시 안고 가야 한다. **핀 드래그 요구가 다시 나오면 그때 이 조합을 검토할 것** — 방법은 "안드로이드 화면 함정"의 ViewAnnotation 항목에 적혀 있다.
       - **시군구 오탭 정밀도가 수용 가능하다.** → **`SGG_MIN_ZOOM` 5 상향은 하지 않는다.** 근거는 남겨둔다: 탭 좌표 포함 판정(`findSggAtPoint`)으로 "누른 곳이 아닌 데가 열리는 것"이 사라졌고, 9.6dp 를 정확히 누르는 어려움만 남았는데 실사용에서 문제되지 않았다. **나중에 불만이 다시 나오면 상수 하나(6→4→5)로 되돌릴 수 있다.**
     - (이하는 진행 당시 기록 — 배경과 함정이 그대로 유효하다.) 갤럭시 실기기에 최신 빌드를 올려 **실제로 써보면서 다듬는다.** 갤럭시 실기기에 최신 빌드를 올려 **실제로 써보면서 다듬는다.** 새 기능을 넓히는 단계가 아니라, 에뮬에서 안 보이던 것(터치 타깃, 실제 GPS, 실제 사진, 스크롤 감, 성능, 권한 다이얼로그)을 잡는 단계다.
     - ⚠️⚠️ **`65f1db5`(2순위)의 "핀 드래그 실기기 검증됨" 기록은 성립하지 않는다 (2026-09-10 정정)**: 그 커밋이 `Marker` → `ViewAnnotation`으로 바꾼 것이고, 코드 증거상 **그 이후로는 육지에서 핀이 보일 수 없다.** 드래그를 얻으려다 **핀 자체를 못 보게 만든 사례**다. (드래그 동작 자체는 살아 있었을 수 있다 — 심볼 히트테스트는 위에 가려져도 잡히므로 "안 보이지만 끌리는" 상태였을 가능성이 있다.) 상세는 "안드로이드 화면 함정"의 ViewAnnotation 항목.
    - **마지막 EAS 빌드 = `d476783` (2026-08-31, build `8d9e98b4`, preview APK)**. 이 빌드로 3차 실기기 검증을 돌렸다. (그 전 빌드는 `131a8cb`(2026-07-09)로 68커밋이나 뒤처져 있었다 — Phase S 전체와 Phase T 다듬기가 이 빌드에 처음 올라갔다.)
     - ⭐ **로컬 release 변형은 debug 키스토어로 서명된다**(`android/app/build.gradle`: `release { signingConfig signingConfigs.debug }`, Expo 템플릿 기본값). 그래서 **구글 로그인이 로컬 빌드에서도 그대로 동작한다** — 로컬 debug 키스토어 SHA-1이 이미 Google Cloud Console에 등록돼 있기 때문(G-3 5단계). **단 이 APK는 스토어에 올릴 수 없다** — 출시용 서명은 EAS 키스토어로 별도로 간다.
     - **다듬기에는 개발 빌드(dev client)가 맞다** — 고치고 바로 폰에서 확인하려면 Metro 연결이 필요하다. release APK는 JS가 안에 박혀 있어 한 번 보기엔 좋지만 반복 수정에는 안 맞는다.
     - **✅ Phase T 항목별 기록 (전부 완료 — 아래는 경위와 함정 보존용)**:
       - **✅ 3차 검증 통과분**: 로고(콜드 재시작 반복)·탭 아이콘·나침반 제거(1순위 3건) / **미니맵 팬 시 부모 스크롤 차단·핀 드래그·저장 후 위치(2순위)** / 초기 뷰 리셋·유지·재리셋(3순위 A·B·C). **2순위는 에뮬에서 아예 확인할 수 없던 항목인데 실기기 첫 검증에 통과했다.**
       - **✅ [T-1] 프로필 글꼴 배율 — 해결됨 (2026-09-10, 3차 빌드 `d476783` 실기기 확인)**: 갤럭시 글자 크기 **최대**에서 프로필의 글씨가 커지되 **깨지지 않는 것**을 실기기로 확인했다. `d476783`(전역 상한 1.2 + `AppTextInput` 신설 + `username`/`gridTitle` `paddingRight: 8`)이 해결한 것으로 판정.
         - ⚠️⚠️ **재현 조건을 특정하지 못한 채 닫는다.** 이전 세션에서 "실기기에서 실패"라고 기록된 그 상태를 이번에 재현하지 못했다 — 그때 폰의 설정 조합이 무엇이었는지 확정하지 못했으므로, **"고쳐졌다"가 아니라 "지금 조건에서는 안 나타난다"가 정확한 표현이다.** 재발하면 **굵은 글씨(Bold font) 켜짐 여부 + 화면 크게/작게(Screen zoom) 단계**를 글자 크기와 **함께** 볼 것.
           - **⭐ 왜 그 두 개인가**: 우리 상한(`MAX_FONT_SCALE`)은 `Configuration.fontScale` **하나만** clamp한다. 삼성 One UI의 **굵은 글씨**는 `fontWeightAdjustment`(안드로이드 12+)를, **화면 크게/작게**는 density를 움직이는 **별개 축**이라 상한이 전혀 안 걸리고, **에뮬에서 `font_scale`을 아무리 올려도 재현되지 않는다.** 글꼴 배율만 의심하다 빗나간 이유가 여기일 수 있다.
           - 실기기 값을 읽는 법: USB가 안 잡히면 **무선 디버깅**(개발자 옵션 → 무선 디버깅 → 페어링 코드)으로 `adb pair`/`adb connect` 후 `adb shell settings get system font_scale` + `wm density`. 폰 화면만으로 볼 거면 설정 → 디스플레이에서 **글자 크기와 스타일**(슬라이더 칸)·**굵은 글씨**(on/off)·**화면 크게/작게**(칸) 3개.
         - **⭐ 조사에서 찾은 "프로필에만 있는 구조적 차이 2개" (재발 시 여기서 시작할 것)**:
           - ① **`extrabold`는 앱 전체에서 프로필 딱 한 곳뿐이다** (`app/(tabs)/profile.tsx`의 `statNumber`, fontSize 28). 이모지를 빼면 **앱에서 가장 크고 가장 굵은 텍스트**다. "배율 절벽" 항목의 서명이 *"글자가 클수록·굵을수록 먼저 무너진다"* 인데, 그 조건의 최댓값이 정확히 여기다. `paddingRight` 보강은 `username`/`gridTitle`에만 들어갔고 **`statNumber`·`statLabel`·`chipText`에는 없다.**
           - ② **프로필에는 `numberOfLines`가 하나도 없다.** 다른 화면 타이틀은 전부 `numberOfLines={1}`이라 폭이 모자라면 **말줄임(…)** 으로 끝나는데, 프로필 텍스트는 자유 줄바꿈이라 측정/draw 폭이 어긋나면 **끝 글자가 보이지 않는 2번째 줄로 넘어가 통째로 사라진다.** 1.3에서 관측된 증상이 "잘림"이 아니라 `@gp123→@gp12`, `나라→나`, `대한민국→대한민`, `내 기록 8`의 **숫자 소실**이었던 것과 맞아떨어진다.
           - ⛔ **폭 부족은 아니다** — `statItem`은 `flex:1`이라 360dp 기준 약 **105dp**인데 "나라"는 13sp × 2글자 ≈ **34dp**다(3배 여유). 나라 칩의 가로 `ScrollView`도 자식에게 폭 제약을 걸지 않는다. **주신 후보 3개(칩 ScrollView / `flex:1`+`statDivider` / 중앙정렬 텍스트) 중 폭 관련은 전부 기각됐다.**
       - **✅ [T-2] GPS 없이 수동으로 핀을 찍을 수 있어야 한다 — 구현·에뮬 검증 완료 (2026-09-10). 실기기 잔여 3건**: "현재 위치"를 눌러도 **핀이 안 찍히고 게시도 영영 안 열리는** 경우가 있었다(실기기).
         - ⚠️ **"지도 화면"은 지도 탭이 아니라 작성 화면이다** — `app/(tabs)/index.tsx`에는 `expo-location` 사용이 아예 없다. "현재 위치" 칩은 **작성 화면 미니맵 위 하나뿐**이다(`compose.tsx`). 다음에 이 증상을 볼 때 지도 탭부터 뒤지지 말 것.
         - **⭐⭐ 근본 원인 = `getCurrentPositionAsync`에 타임아웃 옵션이 없다.** `expo-location@19.0.8`의 `LocationOptions`는 `accuracy`/`mayShowUserSettingsDialog`/`timeInterval`/`distanceInterval` **4개뿐**이고(`Location.types.d.ts`), JS 구현은 네이티브 호출 한 줄 패스스루다(`Location.js`). **측위가 안 잡히면 promise가 영원히 resolve되지 않는다** — 실내·지하·약전계에서 실제로 일어난다. 그러면 `finally`가 영영 안 돌아 `gpsLoading`이 굳는다.
           - → **`Promise.race`로 밖에서 상한을 씌운다**(`withTimeout`, `GPS_TIMEOUT_MS = 15000`).
           - ⚠️⚠️ **race로 빠져나가도 네이티브 측위는 계속 돌고 나중에 resolve된다.** 그대로 두면 **핀 찍고 글 쓰는 중에 갑자기 위치가 바뀐다** — 이 기능에서 제일 나쁜 버그다. 반드시 **세대 ref(`gpsRequestIdRef`)로 늦은 응답을 무시**할 것(Phase D-2/I의 `requestIdRef` 패턴 그대로). `await` 마다 `isStale()`을 통과시킨다.
           - ⚠️ `finally`에서도 `isStale()`을 봐야 한다 — stale이면 `gpsLoading`은 새 요청(또는 취소)이 이미 소유하고 있다. **반대로 세대만 올리고 로딩을 안 내리면 그 자체가 원래 버그의 재발이다** — 그래서 취소는 `cancelGpsRequest()`가 세대 증가와 `setGpsLoading(false)`를 **같이** 한다.
         - **⭐ `canPost`의 `!gpsLoading` → `awaitingFix`로 좁혔다.** 원래 의도(6순위: 곧 덮어써질 캐시 좌표로 저장돼 서버 시군구 판정이 어긋나는 것 방지)는 **그대로 두고 조건만 좁힌다** — `awaitingFix = gpsLoading && coordSource === 'gps-cache'`. 좌표 출처를 `CoordSource`(`'gps-cache' | 'gps' | 'manual'`)로 들고 있다가, **사용자가 직접 탭·드래그한 핀(`manual`)은 정정이 오지 않으므로 즉시 게시가 열린다.**
           - 타임아웃 시 캐시 핀은 **버리지 않고 남기고 `manual`로 승격**한다(정정이 안 오니 더는 `gps-cache`가 아니다) + "핀이 대략 위치에 있으니 끌어서 맞춰주세요" 안내. 핀이 아예 없으면 기존 실패 문구.
           - 측위 중 칩은 `disabled`가 아니라 **"취소"** 로 바뀐다 — `disabled`면 재시도조차 못 했다.
         - **✅ 원래도 정상이었던 것(고치지 않음)**: 권한 거부 경로는 이미 안내 문구 + `gpsLoading` 해제로 **수동 핀 진행이 가능**했다. 미니맵 탭(`onPress`)도 GPS와 무관하게 상시 동작하고(핀 드래그는 2026-09-10에 제거됐다 — 위 ViewAnnotation 항목 참고), "지도를 탭하거나 핀을 끌어 옮기세요" 안내도 **상시 노출**돼 있었다. **즉 수동 핀 기능 자체는 이미 완성돼 있었고, 유일하게 막던 게 `!gpsLoading`이었다.**
         - ⚠️ **보고된 증상보다 실제 피해가 더 컸다**: 보고는 "핀이 안 찍힌다"였는데, 실제로는 **그 상태에서 지도를 탭해 핀을 직접 찍어도 게시가 안 되는** 영구 차단이었다(`!gpsLoading`). **증상을 그대로 재현 조건으로만 받지 말고 그 상태에서 무엇이 더 막히는지 끝까지 따라가 볼 것.**
         - **에뮬 검증 통과분 (2026-09-10)**: 권한 거부 회귀(안내+스피너 해제+수동 진행) / **GPS를 한 번도 성공시키지 않은 상태에서 지도 탭만으로 게시 활성**(핵심 수정) / 취소 칩 / 저장 후 위치 정확(수동 핀 → `country_code=KR`, 서버 트리거가 **옥천군 43730** 판정, `sgg_visits` 자동 증감, 삭제 후 기준선 복구).
           - **stale 차단 4시나리오 전부 통과**: 수동 탭이 이김 / 취소 후 늦은 응답 / 타임아웃 후 늦은 응답 = `stale=true`(또는 race가 이미 확정) → 핀 불변 / **대조군: 무조작 시 `stale=false`로 핀이 실제 갱신됨.** 방법론과 함정은 "에뮬레이터로 검증할 수 없는 것들"의 2026-09-10 항목이 정본.
         - **⛔ 실기기 잔여 3건 (에뮬에서 원리적으로 확인 불가)**:
           - ① ~~핀 시각 확인 + 드래그~~ → **원인이 밝혀져 수정됐다 (4차 실기기, ViewAnnotation → circle 레이어).** 남은 확인은 **"드래그 없이 탭 이동만으로 충분한지"** 뿐이다.
           - ② **캐시 핀 → 정정 대기(`awaitingFix`) + 6순위 회귀** — 에뮬은 `getLastKnownPositionAsync`가 항상 null이라 `gps-cache` 상태 자체가 생기지 않는다. **타임아웃 시 "핀 유지 + 끌어서 맞춰주세요" 안내도 여기서만 확인된다.**
           - ③ **`GPS_TIMEOUT_MS = 15000`이 실내에서 적절한 값인지** — 너무 짧으면 잡힐 측위를 포기하고, 너무 길면 갇힌 느낌이 남는다.
       - **✅ 4순위 — 줌아웃 상태의 시군구 색칠 — 완료 (2026-09-10). `SGG_MIN_ZOOM` 6 → 4, 상수 1줄.**
         - 줌 4~6 구간에서 한국이 화면에 크게 보이는데도 한 덩어리라 시군구 색칠이 안 보이던 문제. **Phase S의 "정확히 교대" 설계는 그대로 두고 임계값만 내렸다** — 회귀 범위가 상수 하나로 좁아진다.
         - **⭐⭐ "시군구 fill을 항상 켜기"는 검토 후 기각했다 (같은 요청이 또 오면 이 조사를 반복하지 말 것)**:
           - ⛔ **나라 방문 신호가 사라진다.** 저줌에서 시군구를 칠하면 z2에서 한국이 **회색 얼룩 + 주황 점 몇 개**가 된다. 바로 옆 태국이 주황 덩어리로 보이는 것과 대비돼 **"거의 안 가본 나라"로 읽힌다.** `INITIAL_ZOOM = 0`이라 **앱을 켜면 첫 화면이 정확히 그 상태다.**
           - ⛔ **두 fill이 공존하면 해안선이 어긋나 나라색이 테두리로 샌다.** 시군구 렌더본은 **해안선 클립 + 3% 단순화**, 나라 경계는 **Natural Earth 50m**이라 해안선이 일치하지 않는다. 실측에서 z5에 **서·남해안 전 구간에 핑크 실선**이 드러났다. **Phase S의 "정확히 교대"가 원래 막고 있던 문제**이고, 임계값만 내리면 재현되지 않는다(두 fill이 절대 공존하지 않으므로).
           - ⭐ **"줌아웃 시엔 시군구를 합친 단순 폴리곤을 쓰자"는 대안 = 이미 `country-fill-kr`이 그것이다.** 별도 union 폴리곤을 만들 필요 없다.
         - **⭐ 성능은 이 결정의 제약이 아니다 (실측)**: 현행 / 항상 켜기(minzoom 0) / 임계값 4 **세 조건 전부 Choreographer 프레임 드롭 0건**(팬 6~8회 + 줌 6회), MapLibre 경고 0건. **이유가 중요하다 — `minzoom`은 레이어(그리기)만 끄고 소스는 모든 줌에서 이미 로드·타일링된다.** 즉 718KB/32,213좌표 파싱·타일 생성 비용은 지금도 이미 지불 중이고, 켜서 늘어나는 건 저줌 그리기 비용뿐인데 한국이 9~36dp라 미미하다.
         - **⭐ 시각 하한선 = z4** (한국 경도폭 6.33°, 512dp 타일, 화면 411dp 기준 — 계산과 실측이 일치): z2 **2.4dp** 얼룩 / z3 **4.8dp** 점 / **z4 9.6dp 식별 가능** / z5 19dp 선명 / z6 38dp.
         - ⚠️⚠️ **부수 효과 — 탭 분기가 z4부터 바뀐다. 그리고 z4 오탭은 "이론상 가능"이 아니라 실제로 일어난다.** 이제 줌 4~6에서 한국을 탭하면 나라상세가 아니라 **시군구 상세**로 간다(꺼진 레이어는 탭 후보가 아니라는 기존 원리 그대로). z4에서 시군구 하나가 9.6dp라 권장 터치 타깃(44dp)보다 훨씬 작다.
           - **회귀 검증 중 실제로 재현됐다 — 충주시를 노리고 탭했는데 정선군이 열렸다.** 복구 경로인 **S-6a의 브레드크럼 `대한민국 · 강원특별자치도` 링크가 정상 작동**하는 것도 같이 확인했다.
           - **⭐ 나중에 오탭 불만이 나오면 임계값을 5로 올리는 선택지가 있다**(시군구 하나가 19dp로 두 배). 그러면 개선 구간이 z5~6 한 단계로 줄지만 탭은 훨씬 안정된다. **상수 하나라 되돌리는 비용이 거의 없다는 게 이 설계의 장점이다.**
           - **✅ 오탭 정밀도 수정 완료 (2026-09-10) — 진짜 원인은 히트박스 크기가 아니라 `features[0]`을 그대로 쓴 것이었다.**
             - 네이티브는 탭 지점 기준 **44×44dp 사각형**으로 질의한다(`MLRNPressableSource.kt`의 `DEFAULT_HITBOX = RectF(22,22,22,22)` → `onMapClick`이 그 RectF로 `queryRenderedFeatures`). 줌 4에서는 시군구 하나가 9.6dp라 **한 번 탭에 20개 이상이 후보로 들어오는데, 그 순서를 우리가 정하지 않는다.** 그래서 엉뚱한 게 열렸다.
             - ⛔ **`hitbox` 는 JS에서 줄일 수 없다** — 네이티브 컴포넌트 prop 에만 있고 공개 `GeoJSONSource` props 에는 노출돼 있지 않다.
             - ✅ **해법: `PressEvent.lngLat`(탭 지점)으로 직접 포함 판정**(`lib/sggGeo.ts`의 `findSggAtPoint`). `lib/countryFromCoord.ts`의 `getCountryFromCoord`와 **같은 패턴**이고(turf `booleanPointInPolygon`), 그쪽은 242피처/13.8K좌표를 작성 화면 탭마다 이미 돌리고 있다 — 시군구는 230피처/32K좌표로 2.3배뿐이고 탭할 때만 돈다. 경계 밖(바다·국경)은 `features[0]` 폴백이라 기존 동작이 회귀하지 않는다.
             - **판정은 화면에 그려지는 렌더본(클립+3%)으로 한다** — 사용자가 본 도형과 판정 도형이 어긋나지 않게.
             - ⚠️⚠️ **이 수정은 오탭을 0으로 만들지 않는다.** **"누른 곳이 아닌 데가 열리는 것"만 없앤다** — 9.6dp 목표를 손가락으로 정확히 누르는 어려움은 그대로다. **실기기에서 여전히 답답하면 그때 `SGG_MIN_ZOOM`을 5로 올리는 것을 검토할 것.**
             - 검증(에뮬, z4): 주황 시군구 2곳 탭 → **의성군·구례군** 정확히 열림 / **대조군**으로 회색 지역 탭 → **거제시**(회색·기록 0)가 열려, "가장 가까운 색칠된 것"을 고르는 게 아니라 실제 탭한 폴리곤을 연다는 것까지 확인.
           - **⭐ `onPress` 를 주지 않으면 그 소스는 히트테스트에서 완전히 빠진다** — `GeoJSONSource.js`의 `hasOnPress: !!props.onPress` → 네이티브 `hitbox` getter 가 `null` → `onMapClick`의 `val hitbox = … ?: continue`. **부작용 0으로 "표시 전용" 레이어를 만들 수 있다.**
         - **⛔ 실기기 검증 항목 (에뮬에서 미검증)**: **색을 바꾸고 지도로 복귀했을 때 새 색이 반영되는지** — 회귀 검증 중 에뮬이 `SIGABRT(RenderThread)`로 죽고 이어서 **JWT 만료(`PGRST303`)** 로 세션이 끊겨 이 왕복만 못 봤다(런처까지 ANR인 degraded 상태였다).
           - **리스크는 낮다**: 바뀐 건 `sgg-fill`의 `minzoom` 하나뿐이고 색칠 바인딩(`useFocusEffect(loadAllVisited)` + `buildSggFillColor`)은 **줌 임계값과 완전히 독립적**이다. 새 임계값에서 **z4.2·z6.2 양쪽 모두 색칠 4곳이 정확히 렌더**되는 것과, 시군구 상세를 다녀온 뒤 지도 복귀 시 색이 유지되는 것까지는 확인했다. **미검증은 "색을 실제로 바꾼 뒤의 왕복"뿐이다.**
         - 라벨(`SGG_LABEL_MIN_ZOOM`)은 **6 그대로** — 줌 4~6에서는 색만 보이고, 6부터 이름이 붙는다.
       - **✅ 5순위 — 시군구 상세의 "기록 추가" FAB — 완료 (2026-09-10)**. `lib/sggGeo.ts` 신설 + compose 파라미터/카메라/복귀 분기 + FAB + 미니맵 경계선.
         - **⭐ `returnTo` 설계**: compose에 `returnTo`(`'country'|'sgg'`) / `sggOsmId` / `sggName` 선택 파라미터를 더했다. **생략하면 `'country'`라 나라상세 경로는 코드가 그대로다** — 회귀 표면이 파라미터 배선뿐이다.
           - **복귀는 "들어온 화면으로" 고정한다.** 핀을 다른 시군구·다른 나라로 옮겨 저장해도 마찬가지. 근거: **저장된 글이 어느 시군구인지 클라이언트가 모른다** — `sgg_id`는 서버 트리거(`set_post_sgg`)가 계산하고 `savePost`는 `Promise<void>`라 아무것도 안 돌려준다. 알아내려면 저장 후 추가 조회 + `sgg_id→osm_relation_id` 역매핑이 필요한데, **핀을 일부러 밖으로 옮긴 드문 경우를 위해 상시 쿼리를 늘릴 값어치가 없다.**
           - ⚠️ **안 넘기면 어떻게 되는지 (소스로 확정)**: `dismissTo` = `POP_TO`이고, 대상이 스택에 없으면 `@react-navigation/routers`의 `StackRouter.tsx`가 *"remove the current route and add the new one"* 분기로 간다. 크래시는 아니지만 `지도 → 시군구 → 나라상세`라는 어긋난 스택이 된다.
           - **나라 판정은 손대지 않았다** — `entryCc`/`sggOsmId`는 **카메라와 복귀에만** 쓰이고, 헤더·저장은 언제나 `countryMatch`(= 핀 좌표 파생)다. 그래서 시군구에서 열어 핀을 일본으로 옮기면 헤더가 `● 일본`이 되고 `country_code=JP`로 저장된다(실측 확인). **시군구 진입 시 `cc`를 아예 넘기지 않는다.**
         - **⭐ 카메라 = `bounds` (고정 줌 아님)**: `initialViewState`가 `bounds`를 받는다(v11 `Camera.d.ts`, 형식 `[west, south, east, north]`). **시군구 경도폭이 74배 차이나서**(대구 중구 0.026° ↔ 옹진군 1.928°) 고정 줌이면 한쪽은 화면 밖, 한쪽은 점이 된다. bbox는 번들된 `sgg_kr_render.json`에서 런타임 계산 — **새 데이터 파일도 재생성도 없다**(지도 탭이 같은 모듈을 import 하므로 번들에 한 번만 들어간다).
         - **⚠️⚠️ 군도 bbox — `padding`은 해결책이 아니라 악화 요인이다.** padding을 키우면 지도가 그만큼 **더 축소**돼 육지가 더 작아진다(16dp만 준다). **진짜 병목은 미니맵 높이 200dp**다 — 옹진군 bbox는 약 170km×107km인데 미니맵이 411×200dp라 세로가 먼저 걸린다.
           - **그래도 bbox를 그대로 두는 이유**: `bbox/육지면적 > 5`인 시군구는 **10/230뿐**이고(>10은 3개), 넓게 열리는 건 **틀린 게 아니라 실제 관할 범위**다. **"가장 큰 섬에 맞추기"나 "줌 하한 clamp"는 앱이 사용자가 간 섬을 추측하는 것이고, 틀리면 사용자가 한참 팬해야 한다 — 전체를 보여주고 사용자가 핀치로 좁히는 것보다 나쁜 실패다.**
           - 실측: 옹진군 bbox/육지 **100.5배**(육지 183km², 파트 31개) / 신안군 23.4배(파트 95개) / 여수시 11.1배.
         - **⭐ 미니맵에 그 시군구 외곽선 한 겹 (없으면 기능이 반만 쓸모 있다)**: compose 미니맵은 `PICKER_MAP_STYLE`(`sources:{}`) + 나라 fill 뿐이라, bounds로 확대해 열면 **화면이 균일한 회색 한 덩어리가 되어 어디까지가 그 시군구인지 알 수 없다.** "확대는 됐는데 그 시군구가 안 보이는" 절반 상태였다. `filter ['==', ['get','osm_id'], id]`로 **진입한 하나만** `line`으로 그린다(fill은 안 준다 — 핀 색과 섞인다). 나라 경로에서는 소스 자체가 렌더되지 않는다(실측 확인).
         - **죽은 `nm` 파라미터 제거**: 나라상세가 `push('/compose', {cc, nm})`로 넘기는데 compose가 읽지 않았다. `dismissTo`에서 나라상세로 넘기는 `nm`은 **나라상세가 실제로 쓰므로 그대로 둔다.**
         - **검증(에뮬)**: 나라 경로 회귀(진입·카메라·저장 복귀·경계선 없음) / 시군구 경로(FAB·bounds 카메라·핀에서 나라 파생·저장 후 시군구 복귀·뒤로 스택) 전부 통과. DB로 서버 트리거가 **예천군** 판정하는 것까지 확인 — bounds 카메라가 정확히 맞았다는 증거다. 검증 후 기준선 복구.
         - **⛔ 실기기 검증 항목**: ① **군도(옹진군·신안군)에서 기록 추가** — 육지가 너무 작으면 **미니맵 높이 200→260dp 완화책**을 검토할 것(레이아웃 변경이라 미뤄뒀다). ② **시군구 경계선이 실기기에서 잘 보이는지**(선 굵기 2, 액센트 주황).
       - **✅ 지도 탭에 내 글 핀 표시 + 설정 토글 — 완료 (2026-09-10)**. `lib/mapPins.ts` / `hooks/use-map-pins-setting.ts` / 지도 탭 circle 레이어 / 설정 "지도" 섹션.
         - **핀 디자인은 흰 원 + 어두운 테두리** — 회색(미방문)·주황/보라(색칠) 배경 양쪽에서 대비가 유지된다. 작성 화면 핀(주황)과 달리 **색칠 위에 얹히므로 주황을 쓰면 묻힌다.**
         - **검증(에뮬)**: 내 글 8개 전부(KR 4·AU 1·TH 1·US 2) / **색칠된 시군구 위에서도 또렷** / **z0·z6.2·z10 전부 표시**(임계값 없음) / 토글 끄면 사라지고 켜면 복귀 / **앱 재시작 후 유지**(설정 화면 UI도 저장값을 정확히 읽음) / 탭 왕복 반영 / **핀이 있는 상태에서 시군구 탭 정상**(4번 회귀 — 핀에 `onPress` 없음).
         - ⭐ **덤으로 4순위의 미검증 항목(색칠 왕복)이 여기서 해결됐다** — 검증 중 충주시 색이 보라(`#8b5cf6`)로 바뀐 걸 발견하고, 앱에서 주황으로 되돌리며 **지도 복귀 즉시 반영되는 것**을 확인했다. 기준선도 함께 복구.
         - 아래는 착수 전 확정한 설계 근거(그대로 유효):
         - **내 글만** 표시한다. `posts_with_coords` 뷰(`id, user_id, location, caption, visibility, taken_at, created_at, country_code, place_label, lng, lat`)를 `user_id` 로 필터. ⚠️ **이 뷰에는 `sgg_id` 가 없다** — S-3에서 컬럼을 추가했지만 뷰는 S-1 재생성 시점 목록으로 고정돼 있다. 필요해지면 뷰를 다시 만들되 **`security_invoker = true` 유지가 생명**.
         - **렌더는 circle 레이어를 재사용하고 `<Map>` 자식 중 마지막에 선언**한다(색칠 fill·라벨보다 위). 위 ViewAnnotation 항목의 교훈 그대로.
         - **⭐ 핀은 탭 불가로 둔다.** 핀 소스도 같은 44dp 히트박스를 받으므로, 탭 가능하게 하면 **핀에서 22dp 이내에서 시군구 탭이 죽는다** — 방금 고친 오탭 문제를 되살리는 셈이다. `onPress` 를 안 주면 히트테스트에서 완전히 빠진다(위 항목). 그 글로 가는 경로는 이미 나라상세·시군구상세 그리드와 프로필 그리드에 있다.
           - **승급 경로**: 나중에 탭이 필요해지면 **핀 핸들러 하나가 거리로 분기**하면 된다 — 탭 지점이 핀에 충분히 가까우면 게시물 상세, 아니면 `findSggAtPoint()` 를 재사용해 시군구 상세로. 오탭 수정이 그 토대다.
         - **⭐ 줌 임계값을 두지 않는다.** 핀이 작고(반지름 5~6) "색칠 위에 정확한 지점"이 이 기능의 목적이라 세계 뷰에서도 보이는 게 맞다. **무엇보다 임계값은 "조용히 안 보이는 모드"를 하나 더 만든다** — 핀이 안 보이는 문제로 빌드 한 사이클을 날린 직후에 같은 종류를 또 만들 이유가 없다.
         - **클러스터링 승급 기준**: 라이브러리가 네이티브로 지원한다(`cluster`, `getClusterExpansionZoom`). **내 글이 200개를 넘거나, 한 화면에서 핀이 겹쳐 개수를 셀 수 없어 보이기 시작하면** 그때 켠다. 지금은 8개.
         - **설정 토글은 AsyncStorage**: `@react-native-async-storage/async-storage` 가 **이미 의존성**이다(supabase 세션 저장소로 사용 중, `lib/supabase.ts`) — 도입 비용 0. `profiles` 컬럼은 마이그레이션 + RLS + 네트워크 왕복이 붙고, **"이 기기 지도에 핀을 띄울지"는 계정 속성이 아니라 기기별 표시 설정**이라 성격도 안 맞는다. 기기 간 동기화가 안 되는 건 오히려 자연스럽다.
       - **⛔ [기존 결함] compose 취소가 나라상세가 아니라 지도로 간다 (2026-09-10 발견, 이번 변경과 무관)**: 5순위 회귀 검증 중 발견하고 **`git stash`로 원본에 돌려 A/B 했더니 원본에서도 동일**했다.
         - **원인은 중첩 네비게이터다** — `compose`는 루트 스택 화면이 아니라 **`(tabs)` 안의 탭 화면**(`href: null`)이라, `push('/compose')`가 루트 스택을 `(tabs)`로 되돌리고 탭만 바꾼다. 그래서 `router.back()`이 나라상세가 아니라 이전 탭 상태로 간다.
         - **고치려면 취소도 `returnTo` 분기를 타게 하면 된다**(`back()` → `dismissTo(진입 화면)`). 다만 **나라 경로의 현재 동작이 바뀌므로 별도 결정이 필요**하다 — 이번엔 범위 밖으로 두고 손대지 않았다.
  6. ⏸️ **보류: 장소검색(지오코딩) — Phase R** (2026-08-23 보류 결정). 실기기 다듬기를 먼저 한다. 착수하게 되면 **지오코딩 제공자 라이선스·쿼터 조사가 선행**이다(Nominatim의 사용량 정책 등 — Phase S와 같은 유형의 판정 건).
  ...
  7. **출시 준비**: 구글 플레이 콘솔 개발자 등록($25) + AAB 빌드(EAS, `preview` 프로필은 APK라 별도 프로필 필요) + 스토어 등록 자료(스크린샷, 설명, 개인정보처리방침 URL) 준비
...
- **완료 (C-1 ~ Phase D)**:
  ...
  - Phase K: **Pretendard 폰트 적용 완료**(2026-07-18) — `constants/theme.ts`의 오랜 TODO(`// TODO: Pretendard 폰트 로드 후 여기에 fontFamily 추가`) 청산. 인프라(1단계)와 화면 교체(2단계)로 나눠 진행.
    ...
    - **원칙(향후 화면에도 적용)**: 새 화면은 `'react-native'`가 아니라 `'@/components/AppText'`에서 `Text`를, **`'@/components/AppTextInput'`에서 `TextInput`을** import할 것(후자는 2026-08-31 신설 — 배경은 아래 "안드로이드 화면 함정"의 글꼴 배율 4층 구조 참고). `fontWeight` 스타일 키는 이 프로젝트에서 금지 — 굵기는 항상 `fontFamily: theme.fonts.*`로 지정.
  ...
  - Phase S: **🎉 한국 시군구 색칠 — 전체 완료 (2026-07-31 조사 → 2026-08-18 완료)**. 세계는 나라 단위 유지, **한국만 시군구 단위로 세분**. 지도 색칠은 진입점이라는 제품 정의는 그대로.
    ...
    - **⭐⭐ ODbL 의무 3개와 충족 방법 (2026-08-16 조문 검증 완료 — 사용자가 원문 재확인)**:
      ...
      - **⛔ 차단 조건 — 이 충족은 "repo가 public"에 전적으로 의존한다**: 저장소를 **private으로 전환하면 그 순간 4.6 위반**이 되고, 경계 파일을 받을 별도 공개 경로를 반드시 따로 마련해야 한다. **잊히기 매우 쉬운 조건이라 차단 조건으로 박아둔다** — repo 공개범위를 건드리는 작업을 할 때 이 항목을 먼저 볼 것.
        - **S-5d부터는 앱이 이 repo를 직접 링크한다** (`app/map-credits.tsx` → `github.com/26-park/colormap/tree/main/data/kr-sgg`). 그래서 private 전환은 이제 라이선스 위반인 동시에 **출시된 앱 안에 죽은 링크를 만든다.** 링크 URL을 바꿀 때도 이 화면을 같이 고칠 것.
    ...
    - **S-4 완료 (2026-08-16, 마이그레이션 `20260816110000_sgg_visits.sql` + `20260816110001_sgg_visits_backfill.sql`)** — 시군구 방문 테이블 + RLS + 동기화 트리거.
      ...
      - **⭐⭐ B1 존재 증명 — 반증 3단 비교 (실측)**: "비공개 글만 있는 종로구를 비친구가 볼 때"
        ...
        - ⭐ **시군구 상세의 "모두" 탭이 posts RLS(`can_view_post`)가 화면을 통해 검증된 첫 지점이었다** (S-6b 2계정 실측: 의성군 private 차단 + 충주시 public 노출 대조). ⚠️ **그 탭은 2026-09-10에 숨겨졌다 — 아래 "모두 탭 숨김" 항목 참고.** **`sgg_visits` 정책은 여전히 잠자는 방어** — 남의 지도/프로필을 보는 화면이 생길 때 깨어난다.
    ...
    - **S-5d 완료 (2026-08-17)** — 설정 → **"지도 데이터 출처"**(`app/map-credits.tsx`, ODbL 4.3 고지). `settings.tsx`의 "앱 정보" 카드에 행 추가, 같은 뒤로가기+타이틀 헤더 패턴. 링크는 `Linking.openURL`(에뮬에서 Chrome 실행 확인).
      - 3개 섹션: **시군구 경계·해안선**(© OpenStreetMap contributors + ODbL 전문/OSM 저작권 링크) / **나라 경계**(Made with Natural Earth., 퍼블릭 도메인 — **표기 의무 없지만 원칙대로 표기**) / **가공 데이터**(repo `data/kr-sgg` 링크).
      - ⭐ **repo 링크를 넣은 이유**: ODbL 4.6은 repo가 public이라 이미 충족돼 있지만, **APK만 가진 사용자는 가공본이 어디 있는지 알 방법이 없다.** 링크가 있어야 "수령인에게 제공"이라는 취지가 실제로 성립한다.
    - **🎉 S-6 완료 (2026-08-18)** — 시군구 상세. 마이그레이션 **0개**(S-3/S-4에서 테이블·RLS·트리거·인덱스가 이미 준비돼 있었다). 3단계 분할, 단계마다 에뮬 검증 후 커밋: S-6a `810b831` / S-6b `b1a679e` / S-6c `9c3082e`.
      - **S-6a 진입 경로 + 뼈대** (`app/sgg/[osmId].tsx`, `lib/sidoNamesKo.ts`, 지도 `onPress`)
        - **라우트 키 = `osm_id`**(= `sgg.osm_relation_id`). 지도가 탭 순간 쥐는 값이 렌더 GeoJSON의 `properties.osm_id`뿐이라 **uuid로 라우팅하면 push 전에 네트워크 왕복이 생겨 탭이 늦어진다.** 색칠 바인딩(S-5c)·경계 갱신 매칭 키도 이미 `osm_relation_id`라 일관된다. `sgg_code`는 인천 4개가 null이라 라우트 키로 못 쓴다. 화면 진입 후 단건 조회로 **uuid를 얻어 이후 쿼리는 전부 uuid**로 나간다.
        - **`sgg` 소스에 `onPress` 한 줄만 추가 — 줌 분기 코드 없음.** 근거는 위 "탭 분기는 줌을 직접 계산할 필요가 없다" 항목.
        - **브레드크럼 `대한민국 · 시도`** — 줌 6 이상에선 나라(KR)상세로 갈 길이 사실상 없으므로(해안선 22dp 폴백뿐) `대한민국`을 링크로 둔다. 실측 확인: 스택이 `지도 → 시군구 → 나라`로 쌓이고 뒤로가기가 자연스럽다.
        - **시도 라벨은 `sgg_code` 앞 2자리 → 17종 매핑**(`lib/sidoNamesKo.ts`). 시군구 **이름 중복이 심해**(중구 5, 동구 5, 서구 4, 북구 4, 남구 4) 라벨 없이는 어디인지 알 수 없다. **코드가 없는 인천 신설 4개는 `osm_relation_id`로 예외 매핑**(이름 매칭 금지 원칙 — 울산 수기 보정과 같은 패턴). **세종은 라벨과 이름이 같아 생략**한다.
      - **S-6b 그리드** — `posts.eq('sgg_id', uuid)` + mine이면 `user_id`, 3열. `resolveMediaUrls`/`requestIdRef`/`loadedPostsOnceRef`/`ErrorView` 전부 나라상세에서 이식. `useFocusEffect` 의존성에 **`sggId`를 넣은 덕에 경계 조회가 끝나는 순간 첫 로드가 자동으로 돈다**(별도 트리거 불필요), `activeTab`은 Phase I와 같은 원리로 탭 전환 재조회가 된다.
        - **검증 대조쌍이 중요했다**: `test`로 **의성군(gp123 private) → 0건 / 충주시(gp123 public) → 1건**. "다 안 보이는 것"과 "선별해서 막는 것"은 이 대조가 있어야 구분된다.
      - **S-6c 색 선택** — G-2 패턴 이식. 잠금은 `posts` count(head, `sgg_id`+`user_id` → `posts_sgg_idx` 적중), 실패 시 `null` 유지로 **fail-closed**하고 문구를 "기록 없음"/"확인 실패"로 분기. 저장은 `sgg_visits.update(...).select('color')`뿐 — **INSERT/DELETE 경로 없음**(S-4 트리거 전담), **0행을 성공으로 취급하지 않는다.** 지도 복귀 갱신은 이미 있는 `useFocusEffect(loadAllVisited)`가 처리해 추가 코드가 없었다.
      - ⭐ **이식할 때 원본에 없던 제약이 생길 수 있다 (실제 사례)**: 잠금 힌트 토스트 배경을 나라상세와 같은 `rgba(0,0,0,0.75)`로 뒀더니 **아래 브레드크럼 글자가 비쳐 보였다.** 나라상세는 헤더 아래가 비어 있어 없던 문제다 → `0.92`로 올려 해결. **G-2/그리드처럼 검증된 패턴을 옮길 때도 "새 화면에만 있는 요소"와의 충돌을 눈으로 볼 것.**
      - **범위 밖으로 명시하고 뺀 것**: **"기록 추가" FAB** — `compose.tsx`가 저장 후 무조건 `router.dismissTo('/country/[cc]')`로 복귀하므로(`app/(tabs)/compose.tsx`), 시군구에서 열면 스택에 없는 화면으로 pop을 시도하게 된다. 작성 진입점은 나라상세 하나로 유지하고, 필요해지면 `returnTo` 파라미터를 넣는 별도 작업으로 다룰 것. / 팔레트 시트 공용 추출(백로그) / 나라상세 수정.
    ...
    - **단계 분할**: **S-0 라이선스 확정 ✅ 완료**(2026-08-16 OSM/ODbL로 소스 재확정) → **S-1 city 잔재 정리 ✅ 완료**(커밋 `1f17991`) → **S-2 ✅ 완료**(추출 → **세종 병합(필수)** → 코드 보정 → 개수·속성 검증 → 단순화 실험/용량 측정 → 경계 테이블+적재) → **S-3 ✅ 완료**(경계 테이블+`posts.sgg_id`+판정 트리거+backfill, 마이그레이션 `20260816100000`) → **S-4 ✅ 완료**(`sgg_visits`+RLS+동기화 트리거+backfill, `20260816110000`/`110001`) → **S-5 ✅ 완료**(a 해안선 클립 / b 레이어+줌6 전환 / c 색칠 바인딩 / d 출처표시) → **S-6 ✅ 완료**(a 진입·브레드크럼 / b 그리드 / c 색 선택, 마이그레이션 0개). 출처표시 UI는 S-5d에서 완료. **Phase S 전 단계 종료.**
...
- **알려진 갭 (백로그, 별도 작업 — Phase O 4단계 조사에서 발견)**:
  ...
  - ✅ ~~**프로필 그리드 1페이지 / 나라 필터 칩 / filteredCount가 마운트 1회라 stale**~~ **해소 (2026-08-26, Phase T 2순위)**. 실기기에서 "미국 글을 썼는데 칩이 안 생긴다"로 실제 증상이 드러나 처리했다. 방식은 아래 참고.
    - **칩 + `filteredCount`는 `useFocusEffect`로 무조건 갱신** — 페이지네이션과 무관해 부작용이 없다. 필터가 없을 때는 통계의 전체 기록 수와 조건이 같으므로 **쿼리를 아낀다**(포커스당 추가 쿼리는 칩 RPC 1개뿐).
    - ⭐ **그리드는 "개수가 바뀌었을 때만" 리셋한다.** 포커스마다 무조건 재조회하면 3페이지까지 스크롤한 상태에서 게시물 상세만 보고 돌아와도 목록이 1페이지로 줄고 스크롤이 튀며 스피너가 번쩍이고, 대표사진 signed URL을 매번 다시 발급하게 된다(= "페이지 단위로만 발급" 원칙 위반). 포커스에서 이미 세고 있는 개수를 신호로 쓰므로 **평상시 추가 비용이 0**이다.
  ...
  - **기록 추가 FAB가 두 곳에 중복 (Phase T 5순위에서 발생, 2026-09-10)**: `addFab`/`addFabPlus`/`addFabText` + Pressable 이 나라상세(`app/country/[cc].tsx`)와 시군구상세(`app/sgg/[osmId].tsx`) 양쪽에 같은 모양으로 있다. **팔레트 시트에 이어 두 번째 복사다.** 넘기는 params 만 다르다(나라 = `cc` / 시군구 = `returnTo`+`sggOsmId`+`sggName`). **세 번째 사용처가 생기면 그때는 미루지 말고 추출할 것** — 아래 팔레트 시트와 같은 기준.
...
- ⚠️ **출시 전 순차 착수 (로드맵에 포함)**: 친구(상호 수락) → 좋아요 → 댓글 → 한국 시군구 색칠(Phase S) → **⭐ 남은 것: 장소검색(지오코딩)**. 순서·배경은 "현재 단계"의 "⭐ 출시 시점 방향 결정" 참고.
...
- ⚠️⚠️ **"모두" 탭 숨김 (2026-09-10) — RLS의 화면 검증 지점이 사라졌다**: 나라상세·시군구상세의 "내 기록 / 모두" 세그먼트를 `constants/features.ts`의 **`SHOW_ALL_POSTS_TAB = false`** 로 감췄다. v1에서 남의 공개 글을 UI에 노출하지 않기로 한 결정이다.
  - **숨긴 것이지 지운 것이 아니다** — `activeTab` 상태와 `'all'` 조회 분기, **공개범위 3단 체계와 RLS 전부 손대지 않았다.** 상수를 `true`로 되돌리면 예전 동작이 그대로 돌아온다. 나중에 탐색 기능을 만들 때 되살릴 계획.
  - ⛔ **그런데 그 탭이 `can_view_post`가 실제 화면을 통해 검증되던 유일한 지점이었다** (S-6b의 2계정 대조: 비공개 글만 있는 의성군 0건 / 공개 글이 있는 충주시 1건). **숨긴 뒤로는 SQL 하네스(`scripts/verify-*.sql`)로만 검증된다** — 정책 자체는 그대로이므로 보호는 유지되지만, **"앱이 실제로 그 정책을 타고 있는가"를 눈으로 확인할 방법이 없어졌다.**
  - **⭐ 다시 화면 검증 지점이 생기는 시점 = 친구 프로필/지도 보기.** 그 화면을 만들 때 2계정 대조(친구의 비공개 글이 안 보이는지)를 **반드시** 넣을 것 — `sgg_visits`/`country_visits`의 잠자던 SELECT 정책도 그때 같이 깨어난다.
...
- ⭐ **검증 후 정리 체크리스트 (에뮬 2계정 시나리오를 돌렸으면 매번 이 순서로 복구)**:
  ...
  5. **`sgg_visits` — 바꾼 색을 `update sgg_visits set color = '#ff6a2b';`로 원복 (4행 유지)** ← 2026-08-18 추가(Phase S-6)
     - ⭐ **행 수가 바뀌었으면 그 자체가 버그 신호다** — 앱에는 `sgg_visits` INSERT/DELETE 경로가 없다(S-4 트리거 전담). 색만 바뀌어야 정상.
  6. 테스트로 만든 `posts`가 있으면 삭제 (G-1 트리거가 `country_visits`를, S-4 트리거가 `sgg_visits`를 알아서 정리한다 — 직접 지우지 말 것)
...
## ⚠️⚠️ 에뮬레이터로 검증할 수 없는 것들 (2026-08-28, 2차 실기기에서 3건 연속 실패하고 배움)

**1차 실기기 보고 3건(탭 라벨 잘림·로고 잘림·우상단 동그라미)을 "에뮬에서 고쳐진 것 확인"으로 판정해 빌드했는데 실기기에서 전부 그대로였다.** 빌드 커밋(`6f102d0`)의 파일 내용을 직접 열어 수정이 들어간 것은 확인했으므로 **누락이 아니라 에뮬 검증이 조건을 재현하지 못한 것**이다.

- ⭐ **터치 회전은 에뮬에서 일어나지 않는다** — 마우스로는 두 손가락 비틀기가 안 된다. "한국지도 버튼 뒤 검은 동그라미"의 정체가 **MapLibre 기본 나침반**이었는데, `compassHiddenFacingNorth` 기본값이 true라 **회전했을 때만** 우상단(기본 위치)에 나타난다. 실기기에서는 핀치 줌 중에 손가락이 조금만 돌아가도 회전된다. → `compass={false}` + `touchRotate={false}` + `touchPitch={false}`로 해결(색칠 지도는 북쪽 고정이 맞다).
...
- ⭐⭐ **`dragPan` 함정 — 스크롤 컨테이너 안에 지도를 넣을 때는 반드시 명시할 것**: 작성 화면 미니맵을 조작하면 **부모 `ScrollView`가 같이 움직였다.** JS의 `dragPan`은 네이티브 `scrollEnabled`로 가는데(`MLRNMapViewManager.kt:122`), 이 필드는 `Boolean? = null`로 시작해 **프롭을 실제로 넘겨야 채워진다**. 부모의 터치 가로채기를 막는 코드가 **`scrollEnabled == true`일 때만** 돌기 때문에(`MLRNMapView.kt:568-575`의 `onTouchEvent` → `requestDisallowInterceptTouchEvent`) `null`이면 차단이 안 걸린다.
  ...
  - ~~핀을 끌어 옮기려면 `ViewAnnotation` + `draggable` 을 쓴다~~ ⛔ **이 권고는 철회한다 (2026-09-10) — 그 교체가 핀을 안 보이게 만들었다.** 바로 아래 항목 참고.
- ⭐⭐ **`ViewAnnotation`은 안드로이드에서 오버레이 View가 아니다 — SymbolManager 심볼이라 레이어 순서에 묻힌다 (2026-09-10, 4차 실기기에서 확정)**
  - **증상**: 작성 화면에서 지도를 탭하면 `pickedCoord`는 정상으로 잡혀 나라명·게시 버튼이 살아나는데 **핀만 안 그려진다.** ⭐ **결정적 판별: 바다를 탭하면 보이고 육지를 탭하면 안 보인다** — 폴리곤이 없는 곳에서만 드러난다.
  - **원인**: `ViewAnnotation`(= `MLRNPointAnnotation`)은 자식 View를 오프스크린에서 비트맵으로 구워 **`SymbolManager` 심볼**로 올린다. 그런데 `MLRNMapView.kt`의 `mapLibreMap.getStyle { createSymbolManager(style); … addQueuedFeatures() }` 순서상 **심볼 레이어가 먼저 스타일에 들어가고**, 우리 `Layer` 들은 그 뒤에 `style.addLayer(layer)`(= **맨 위**)로 얹힌다(`MLRNLayer.kt`). 그래서 **불투명한 `fill` 레이어가 핀을 덮는다.**
  - **`Marker`(MarkerView)는 다르다** — `MarkerViewManager.kt`의 `mapView.addView(view)`, 즉 **진짜 안드로이드 View 오버레이**라 스타일 레이어 순서와 **무관하게 항상 위**에 그려진다.
  - **✅ 채택한 해법: 핀을 `GeoJSONSource` + `Layer type="circle"` 로 직접 그린다.** 우리 레이어는 **선언 순서대로 위에 쌓이므로 마지막에 선언하면 곧 최상단**이다 — 인덱스 계산도, 라이브러리 내부 레이어 id 의존도 없다. 기존 핀이 주황 원 + 흰 테두리라 `circle-radius`/`circle-color`/`circle-stroke-*` 로 그대로 재현된다(이미지 에셋 불필요). 라이브러리 문서도 *"Consider using GeoJSONSource and SymbolLayer instead"* 로 권한다.
    - ⛔ `layerIndex`/`beforeId` 로 우리 레이어를 심볼 아래로 미는 방법도 있지만 **기각했다** — 레이어를 추가할 때마다 인덱스를 손으로 관리해야 하고 **어긋나면 조용히 다시 안 보인다.** 방금 겪은 실패 모드가 그대로 재발하는 구조다.
    - 드래그는 이 방식에서 빠진다. **지도를 탭하면 핀이 그 자리로 옮겨가므로**(`handleMapPress`) 200dp 미니맵에선 탭이 오히려 빠를 수 있다 — 실기기에서 써보고 아쉬우면 circle + (안 보이는) ViewAnnotation 조합을 검증해 붙인다.
  - ⚠️ **같은 함정이 지도 탭에도 그대로 있다** — 지도 탭에 핀을 얹을 때(내 기록 위치 표시) `ViewAnnotation`/`Marker` 를 쓰지 말고 이 circle 레이어 방식을 재사용할 것.
- ⭐ **판정 규칙**: **터치 회전 / 폰트 측정 / 시스템 글꼴 배율 / 실제 GPS / 제조사 키보드 / 멀티터치 제스처 충돌**이 얽힌 항목은 **에뮬 통과를 해결 근거로 삼지 말 것.** 에뮬은 "안 고쳐졌다"는 반증에는 쓸 수 있어도 "고쳐졌다"는 증명에는 못 쓴다.
  - ⚠️⚠️ **이 규칙을 알고도 또 어겼다 (2026-08-31, Phase T 6순위)**: 글꼴 배율을 `font_scale 2.0` + `wm density 480`까지 올려 "전 화면 정상"으로 판정했는데 **실기기 갤럭시 최대 배율에서 프로필이 그대로 깨졌다**(T-1). 에뮬에서 조건을 아무리 극단으로 밀어도 **제조사 설정이 실제로 어떤 값을 만드는지는 기기에서 읽어야 안다** — 다음엔 **`adb shell settings get system font_scale`로 실기기 값을 먼저 읽고** 그 값으로 재현할 것. (**후일담**: T-1은 2026-09-10 실기기에서 정상으로 확인돼 닫혔지만 **재현 조건은 끝내 특정하지 못했다** — 위 T-1 항목의 "굵은 글씨·화면 크게/작게" 단서 참고. 즉 이 규칙은 **여전히 유효하다**: 에뮬 통과가 해결 근거가 아니었던 것처럼, 실기기 1회 통과도 원인 규명은 아니다.)

**⭐ 2026-09-10 (T-2 GPS 검증)에서 새로 확인된 에뮬 한계 5가지** — 전부 "에뮬에서 안 되는 걸 자기 코드 탓으로 오해하기 쉬운" 것들이다.

- ⛔⛔ **[정정됨 — 이 항목은 오진이었다 (2026-09-10, 4차 실기기)]** 원래 여기에 *"MapLibre `ViewAnnotation`이 에뮬에서 렌더되지 않는다"* 고 적어 **에뮬 한계로 판정하고 실기기 항목으로 넘겼는데, 실기기에서도 똑같이 안 보였다.** 에뮬 한계가 아니라 **실제 버그**였다. 원인과 수정은 아래 "안드로이드 화면 함정"의 **ViewAnnotation 항목**이 정본.
  - ⭐⭐ **여기서 배울 방법론 (이번 세션에서 실제로 한 사이클을 낭비했다)**: **A/B(원본에서도 재현됨)는 "내 변경 탓이 아니다"만 증명한다.** 거기서 **"그러면 환경(에뮬) 탓"으로 건너뛰면 안 된다** — **"기존 버그"라는 가능성이 그대로 남아 있고, 오히려 그쪽이 더 흔하다.** 이번엔 A/B 결과를 손에 쥐고도 엉뚱한 결론을 내렸고, 빌드를 뽑아 실기기 검증까지 돌린 뒤에야 드러났다.
  - **A/B 뒤에 반드시 물을 것**: "원본에서도 안 된다면, **원본이 언제부터 그랬나?**" — `git log -- <파일>`로 그 기능을 마지막으로 건드린 커밋을 찾아 **거기서 뭘 바꿨는지** 보면 대개 즉시 갈린다(이번엔 `65f1db5`의 `Marker`→`ViewAnnotation` 교체였다).
- ⭐ **`adb emu geo fix`가 먹지 않고 `getCurrentPositionAsync`가 아예 resolve되지 않는다** (30초 타임아웃까지 대기하는 것을 실측). `dumpsys location`을 보면 GPS provider의 last location이 **부팅 시 기본 좌표(37.421998,-122.084)** 그대로다.
  - 역설적으로 **"측위가 영영 안 끝나는" 무한 대기 시나리오는 에뮬에서 자연 재현된다** — T-2가 고친 버그를 실제 expo 호출로 확인할 수 있다.
  - 반대로 **"GPS가 성공하는" 경로는 에뮬에서 만들 수 없다.** 그 경로를 검증하려면 expo 호출 자체를 **결정론적 가짜 좌표를 돌려주는 promise로 임시 대체**할 것. 타이밍이 확정돼 오히려 더 좋은 하네스가 된다(단 expo 연동 자체는 검증되지 않으므로, 그건 위 무한 대기 경로로 따로 확인).
- ⭐ **`getLastKnownPositionAsync`가 항상 `null`이다** → `coordSource === 'gps-cache'` 상태가 에뮬에서는 **아예 생기지 않는다.** 캐시 핀 즉시 표시·`awaitingFix`(정정 대기 중 게시 차단)·타임아웃 시 "핀 유지 + 끌어서 맞춰주세요" 안내는 **전부 실기기 전용 항목**이다.
- ⭐⭐ **딜레이 주입 방법론 — 딜레이는 타임아웃보다 *짧아야* 한다.** `Promise.race`로 상한을 씌운 코드에서 **딜레이 > 타임아웃**으로 잡으면 race가 먼저 확정돼 **늦은 응답이 코드에 도달조차 하지 않는다.** 그러면 "핀이 안 바뀐다"만 보이고 **stale 가드가 실행된 적이 없어 아무것도 증명하지 못한다**(실제로 이렇게 잘못 짰다가 다시 잡았다).
  - **진짜 위험 구간은 "타임아웃 전에 응답이 왔는데 그 사이 사용자가 수동으로 찍은 경우"** 다. 그 구간을 만들려면 딜레이 < 타임아웃이어야 한다.
- ⭐⭐ **대조군 없는 차단 검증은 검증이 아니다.** "막혔다"는 시나리오(stale=true)만 모으면 **전부 무조건 차단하도록 잘못 짜도 전부 통과한다.** 반드시 **정상 경로가 실제로 값을 갱신하는 대조군**(T-2의 S4: `stale=false`로 핀이 일본→대한민국으로 바뀜)을 같이 둘 것 — 그게 있어야 나머지 `true`들이 의미를 가진다.
...
- ⭐ **글자 잘림의 원인을 폭 부족으로 단정하지 말 것 — 시스템 글꼴 배율이 진짜 원인인 경우가 있다.** 탭 라벨이 "지도"→"지…"로 잘린다는 보고를 **에뮬 기본(1.0)에서는 재현 못 했고, `adb shell settings put system font_scale 1.5`로 즉시 재현**했다. 사용자 폰의 글꼴 크기가 기본보다 크게 잡혀 있었던 것.
  ...
  - **전역 정책은 4층 구조다 (2026-08-31 갱신, Phase T 6순위)**:
    1. **`components/AppText.tsx`에 기본 상한 `MAX_FONT_SCALE = 1.2`** — 앱 전체 `Text`가 이 파일 하나를 통과하므로 한 곳만 막으면 전역에 걸린다.
    2. **`components/AppTextInput.tsx`에 같은 상한** — `AppText`는 `Text`만 감싸므로 **`TextInput`은 상한이 전혀 안 걸려 있었다**(2026-08-31 발견, 입력란 8곳 전부 무방비). 배율 2.0에서 친구 검색창 placeholder가 **글자끼리 겹쳐 뭉개졌다**("아이디를 정확히 입력하세요" → "아이니들 성왁히 입뎍하세"). 상한을 공유하려고 `MAX_FONT_SCALE`을 `AppText.tsx`에서 export한다. **덤: 대부분의 입력란이 `fontFamily` 지정 없이 시스템 폰트로 렌더되고 있었는데 이것도 같이 해결됐다.**
    3. **탭바는 확대를 끈다** (`tabBarAllowFontScaling: false`) — 높이가 고정(49dp+인셋)이라 늘어날 공간이 없다.
    4. **지도 헤더(로고·한국지도 버튼)도 끈다** (`allowFontScaling={false}`) — 지도 위 고정 오버레이라 커지면 지도를 가리고 글자도 잘린다.
    - ⭐ **새 화면은 `'react-native'`가 아니라 `@/components/AppText`(Text)와 `@/components/AppTextInput`(TextInput)에서 import할 것.** 이 두 통로를 안 지나면 상한도 기본 글꼴도 안 걸린다.
  - ⭐⭐ **배율 절벽 — 1.2는 멀쩡하고 1.3부터 전부 잘린다 (2026-08-31 실측)**: 점진적 악화가 아니다. `font_scale` 1.2에서는 `@gp123`·`나라/기록/친구`·`대한민국` 칩·`내 기록 8`이 전부 온전한데, **1.3에서 한꺼번에 무너진다**(`@gp12`, `나`·`기`·`친`, `대한민`, 숫자 소실).
    - **원인은 폭 부족이 아니다.** 잘린 요소들에 `numberOfLines`도 고정 폭도 없다 — `statItem`은 `flex:1`이라 약 120dp인데 "나라"는 34dp밖에 안 쓴다. **진짜 원인은 아래 "커스텀 폰트 텍스트가 …잘린다" 항목의 Pretendard 측정 폭 문제**이고, 그 오차가 **배율에 비례해 커져서** 임계점을 넘는 것이다.
    - ⛔ **그래서 칩 줄바꿈·통계 세로 배치 같은 레이아웃 대응은 필요 없다. 폭 부족으로 오인해 레이아웃을 뜯지 말 것.**
    - ⚠️ **상한 1.2는 근본 해결이 아니라 회피다.** 증상이 드러나는 지점을 밀어냈을 뿐이고, 폰트를 바꾸거나 측정 문제가 해결되면 **상한을 다시 올릴 수 있다.**
    - **1.2를 고른 근거**: "1.2로 제한" vs "1.3에서 잘려 아예 못 읽음"의 선택이다. **잘린 글자는 확대의 의미가 없으므로 1.2가 실질적으로 접근성이 더 높다.**
    - **상한만으로는 부족했다** — 1.2로 낮춰도 **굵고 큰 텍스트 2곳**(`username` 18 bold, `gridTitle` 16 bold)은 계속 잘렸다. 로고에 썼던 것과 같은 **`paddingRight: 8`(가로 여백)** 을 주니 해결됐다. 측정 폭 문제의 서명은 **"글자가 클수록·굵을수록 먼저 무너진다"** 이다.
    - **검증 조건**: 시스템 배율 2.0 + 화면크기 최대(`wm density 480` = 폭 360dp)까지 확인 — 프로필/설정/친구/게시물상세 전부 정상.
    - ⭐ **상한이 걸린 텍스트는 시스템 배율이 상한 이상이면 렌더 결과가 동일하다** — 그래서 **기기의 실제 최대 배율을 알 필요 없이 에뮬 `font_scale 1.3`으로 재현된다.** (단 상한이 없는 곳은 실제 값이 그대로 곱해지므로 이 성질이 성립하지 않는다.)
    - ⚠️ **`wm density`를 앱이 뜬 상태에서 바꾸면 이전 폭으로 그린 화면이 남아** 헤더가 잘린 것처럼 보인다. **밀도를 바꿨으면 `am force-stop` 후 다시 띄우고 판정할 것** — 실제로 이걸 버그로 오인할 뻔했다.
...
- ⭐ **커스텀 폰트 텍스트가 오른쪽에 공간이 남는데도 마지막 글자가 잘린다 (2026-08-27)**: 지도 헤더 로고가 "Tintrail" → "Tintrai"/"Tintra"로 잘렸다. **컨테이너 폭 문제가 아니고 `letterSpacing`도 원인이 아니다**(음수 letterSpacing을 지웠더니 한 번은 멀쩡했다가 다음 렌더에서 더 심하게 잘렸다) — 안드로이드가 **커스텀 폰트 텍스트 폭을 실제 글리프보다 좁게 측정**하는 간헐적 문제다. `flexShrink: 0` + `paddingRight`로 여유를 줘서 막는다.
  ...
  - ⏸️ **아직 해결로 단정하지 말 것 (2026-08-27 기준)**: 콜드 재시작 **5/5 정상**이고 잘림은 **핫리로드 직후에만** 재발한다(사용자는 겪지 않는 조건). 다만 **원래 보고는 실기기 preview APK**였으므로 조건이 다르다 — **다음 빌드에서 최종 확인**할 것.
- ⭐⭐ **MultiPolygon에 symbol 레이어를 직접 물리면 파트마다 라벨이 붙는다 (2026-08-27, Phase T 4순위)**: 시군구 경계에 그대로 라벨을 얹었더니 섬이 많은 곳에서 이름이 여러 번 나왔다 — **실측: 여수시 6개, 통영시 7개, 고흥군 5개, 거제시 3개.** 자동 배치로는 못 고치므로 **피처당 점 1개짜리 별도 포인트 소스를 미리 만들어야 한다**(`data/kr-sgg/make_labels.py` → `sgg_kr_labels.json`, 30KB).
  - ⚠️ 앵커는 **`representative_point()`** 로 잡는다(PostGIS `ST_PointOnSurface`와 같은 개념). **`centroid`를 쓰면 안 된다** — 해안선이 파인 오목한 시군구에서 도형 밖으로 나간다. 스크립트가 "앵커가 도형 안에 있는지"를 검증한다.
- ⭐ **스타일에 `glyphs` 루트 속성이 없으면 `text-font`는 로컬 시스템 폰트 이름으로 해석된다** (MapLibre 사양). 덕분에 **글리프 서버 없이 한글 라벨이 렌더된다** — 타일 벤더가 없는 이 프로젝트 구조에 그대로 맞는다(외부 의존성·라이선스·네트워크 0). 라벨을 손볼 때 "글리프 서버부터 붙여야 하나"로 새지 말 것.
  - 겹침은 MapLibre 기본 동작(`text-allow-overlap` 기본 false)이 처리한다 — 자리가 없으면 숨고 확대하면 나타난다. 별도 로직 불필요.
...
- ⭐ **"현재 위치"가 느린 건 `getCurrentPositionAsync`가 새 측위를 기다리기 때문이다 (2026-08-28)**: expo-location 공식 문서도 *"빠른 응답이 필요하면 `getLastKnownPositionAsync`를 쓰라"* 고 권한다. **캐시 위치로 핀을 즉시 찍고 실제 측위로 정정**하는 2단계로 바꿨다(`maxAge` 2분 / `requiredAccuracy` 500m로 오래되거나 부정확한 캐시는 배제).
  ...
  - ⭐⭐ **`getCurrentPositionAsync`에는 타임아웃 옵션이 없다 (2026-09-10, T-2)** — `LocationOptions`는 `accuracy`/`mayShowUserSettingsDialog`/`timeInterval`/`distanceInterval` 4개뿐이고 JS는 네이티브 패스스루다. **측위가 안 잡히면 promise가 영원히 resolve되지 않는다.** `Promise.race`로 밖에서 상한을 씌우되, **race로 빠져나가도 네이티브 측위는 계속 돌아 나중에 resolve되므로 세대 ref로 늦은 응답을 반드시 무시할 것** — 안 그러면 사용자가 그새 직접 찍은 핀을 덮어쓴다. 상세는 위 Phase T의 [T-2] 항목.
...
- ⭐ **가드는 "정말 필요한 경우만" 좁게 걸 것 (2026-09-10, T-2에서 배움).** 6순위에서 캐시 좌표 저장을 막으려고 `canPost`에 `!gpsLoading`을 걸었는데, 그 한 줄이 **사용자가 직접 찍은 핀으로 게시하는 경로까지 막았고** 측위가 영영 안 끝나면 영구 차단이 됐다. **좋은 의도의 안전장치가 다른 경로를 막는 전형이다.** 가드를 넣을 때는 ① 막으려는 나쁜 상태를 정확히 이름 붙이고(여기선 "곧 덮어써질 캐시 좌표" = `awaitingFix`) ② **그 상태에만** 걸었는지, ③ 이 가드가 켜진 채로 굳을 수 있는 경로가 있는지를 확인할 것. 넓게 거는 쪽이 안전해 보이지만 **사용자를 막다른 길에 가두는 건 더 나쁘다.**
