...
- **⭐ 다음 세션 시작점**: P0/P1/P2 전부 완료 — 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  1. (우선순위 낮음, 급하지 않음) `expo-modules-core` 직접 의존성 정리 — `expo` 패키지가 재노출하는 API로 교체
  2. 친구 기능 (상호 수락) — ⚠️ 착수 전 "권한/가시성 모델" 섹션의 ⛔ 차단 조건(country_visits RLS)부터 확인
  3. 좋아요
  4. 댓글
  5. 장소검색(지오코딩) / 3D 지구본 토글
...
- **v1 출시 점검 나머지 후보**:
  ...
  - **[P2 조정]**
    ...
    - ✅ 게시물 공개범위 사후 변경 + 계정 공개범위 토글 — 완료 (위 Phase L 참고, 2026-07-19). **P2 전부 완료.**
  - **[P2 마무리 이후 로드맵 — 출시 전 순차 착수]**: (우선순위 낮음) `expo-modules-core` 의존성 정리 → 친구 기능 → 좋아요 → 댓글 → 장소검색/3D 지구본. 배경·순서 근거는 위 "⭐ 출시 시점 방향 결정" 참고 — v1.1 기능이지만 이번엔 출시 후가 아니라 출시 전에 만든다.
  - **[출시]**
