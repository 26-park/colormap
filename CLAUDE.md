...
- **⭐ 다음 세션 시작점 (2026-08-31)**: 로드맵 5번 **Phase T의 "남은 목록"** 부터 — **5순위(시군구 FAB)** (**[T-1] 프로필 글꼴 배율·[T-2] GPS 수동 핀·4순위 줌아웃 색칠은 2026-09-10 완료**). 전체 순서는 아래 로드맵대로 (배경은 "⭐ 출시 시점 방향 결정" 참고).
  ...
  5. ⭐⭐ **진행 중: 출시 전 실기기 다듬기 (Phase T) — 2026-08-23 추가**. 갤럭시 실기기에 최신 빌드를 올려 **실제로 써보면서 다듬는다.** 새 기능을 넓히는 단계가 아니라, 에뮬에서 안 보이던 것(터치 타깃, 실제 GPS, 실제 사진, 스크롤 감, 성능, 권한 다이얼로그)을 잡는 단계다.
     ...
     - **⭐⭐ Phase T 남은 목록 (2026-08-31 3차 실기기 검증 후 확정 — 다음 세션은 여기서 시작)**:
       ...
       - **⏸️ 5순위 — 시군구 상세의 "기록 추가" FAB**: S-6에서 의도적으로 범위 밖에 뒀다. `compose.tsx`가 저장 후 무조건 `router.dismissTo('/country/[cc]')`로 복귀하므로 **`returnTo` 파라미터 설계가 선행**이다(상세는 Phase S-6의 "범위 밖으로 명시하고 뺀 것" 참고). **설계 변경이라 신선한 세션에서 다룬다.**
