...
- **🎉 한국 시군구 색칠 S-5 완료 (2026-08-17)**: 해안선 클립+3% 단순화 / 지도 시군구 레이어+줌6 전환 / 줌 버튼 연타 누적 / 색칠 `osm_id` 바인딩 / 출처표시 화면. Phase S는 **S-6(시군구 상세·색 선택)만 남았다.** 상세는 아래 Phase S 참고.
...
- **⭐ 다음 세션 시작점**: 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  ...
  2. ✅ **좋아요 — 완료 (Phase P)**
  3. ✅ **댓글 — 완료 (Phase Q)**
  4. ⭐ **진행 중: 한국 시군구 색칠 (Phase S)** — **S-0~S-5 완료 (2026-08-16~17)**. 소스 재확정(OSM/ODbL) → 경계 230개 확보·적재 → 서버측 판정 트리거 → 방문 테이블+RLS → 지도 렌더링·색칠·출처표시까지 끝났다. **남은 건 S-6(시군구 상세/색 선택)뿐이다.**
     - **⭐ 다음 세션은 S-6(시군구 상세·색 선택)부터.** 나라상세(`app/country/[cc].tsx`)의 **G-2 패턴을 시군구로 이식**하는 작업이다(게시물 0개면 색 동그라미 비활성 + 인라인 안내, 1개 이상이면 팔레트 오픈, 앱은 `UPDATE`만·INSERT 경로 없음).
       - ⚠️ **진입 경로 설계가 선행돼야 한다** — 지도에서 시군구를 탭했을 때 상세로 가는 길이 아직 없다.
         - ⭐ **정정 (2026-08-18, 네이티브 코드 실사)**: 과거 여기에 "시군구를 탭해도 `/country/KR`로 간다"고 적혀 있었으나 **틀렸다.** 줌 6 이상에서는 `country-fill-kr`이 `maxzoom=6`으로 꺼져 있어 **한국 내륙 탭은 아무 일도 일어나지 않는다.** 다만 `country-border`(line)는 **필터도 줌 제한도 없어 항상 렌더**되므로 **국경·해안선 22dp(기본 히트박스) 이내를 탭하면** `countries`가 히트해 `/country/KR`로 간다. → 이 폴백을 S-6 설계에 그대로 활용한다(바다 쪽 탭 = 나라상세).
         - **탭 분기는 줌을 직접 계산할 필요가 없다**: 네이티브가 소스별로 `queryRenderedFeatures(터치점+히트박스, 그 소스의 레이어 ID들)`를 돌리고(`MLRNMapView.kt:617-650`), 여러 소스가 히트하면 **스타일에서 더 위에 있는 레이어를 가진 소스**가 이긴다(`getPressableSourceWithHighestZIndex`, `:1467`). 꺼진 레이어는 후보가 아니므로 min/maxzoom 교대가 탭 분기에 자동으로 따라온다. `sgg` 소스에 `onPress`만 붙이면 된다.
     - ⚠️ **미결 — 인천 신설 4개 코드**: `sgg_code` null로 두고 진행 중. surrogate id 설계라 아무것도 막지 않으므로 급하지 않다.
     - ⏸️ **미확인 보류 (무해, 다음 로그인 때)**: ① **gp123 4곳 색칠 형식 확인**(충주시·구례군·의성군·예천군) — 렌더 경로 자체는 `test`에 임시 행을 넣어 종로구로 실증했으므로 형식 확인에 가깝다. ② **최대줌 16에서 `+` 비활성** — 최소줌에서 같은 로직을 확인했다.
