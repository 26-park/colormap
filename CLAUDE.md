...
- **⭐ 다음 세션 시작점**: 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  ...
  5. ⭐⭐ **진행 중: 출시 전 실기기 다듬기 (Phase T) — 2026-08-23 추가**. 갤럭시 실기기에 최신 빌드를 올려 **실제로 써보면서 다듬는다.** 새 기능을 넓히는 단계가 아니라, 에뮬에서 안 보이던 것(터치 타깃, 실제 GPS, 실제 사진, 스크롤 감, 성능, 권한 다이얼로그)을 잡는 단계다.
     - **⚠️ 마지막 EAS 빌드는 `131a8cb`(2026-07-09) 기준으로 지금 HEAD보다 60커밋 뒤처져 있다** — 구글 로그인·계정삭제·약관·아이콘·한글화·설정화면·Pretendard·공개범위·`expo-crypto`·친구·좋아요·댓글·**Phase S 전체(시군구)** 가 전부 그 뒤에 들어왔다. **폰에 깔린 게 있어도 그건 7월 9일 앱이다 — 반드시 재빌드.**
