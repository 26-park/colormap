...
- **⭐ 다음 세션 시작점**: 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  ...
  2. ✅ **좋아요 — 완료 (Phase P)**
  3. ✅ **댓글 — 완료 (Phase Q)**
  4. ⭐ **진행 중: 한국 시군구 색칠 (Phase S)** — S-0(라이선스 확정)·S-1(city 잔재 정리) 완료. **다음 세션은 여기부터, 아래 3단계 순서대로:**
     1. **VPN으로 한국 IP 붙어서 vworld 다운로드 재시도** — `https://www.vworld.kr/dtmk/dtmk_ntads_s002.do?dsId=30015`. ⚠️ **해외 IP 차단이 유력한 가설**: 같은 망에서 **data.go.kr은 정상인데 vworld만 502**다(도구·브라우저 양쪽 모두 실패). 먼저 IP를 바꿔보고, 그래도 안 되면 서버 문제로 판단할 것.
     2. **받으면 S-2**: 속성 테이블을 열어 **250행의 정의부터 확인**(시군구 229와 21행 차이 — 아래 Phase S의 ⚠️⚠️ 항목) → 그 결과로 **식별자 매핑 설계**. 229를 전제로 스키마를 먼저 짜지 말 것.
     3. **계속 막히면 2순위 `15059910`(국토지리정보원 기본공간정보, 제1유형) 전환 판단** — 단 **전용 S/W + 영역 신청 절차라 리드타임이 있다.** 전환은 하루 이상 막혔을 때 결정.
     - 파일 없이도 가능한 작업: **S-4 방문 테이블 RLS 정책 초안**(`can_view_post` 경유) — 다운로드가 계속 막히면 이걸 먼저 진행해도 된다.
  5. 장소검색(지오코딩)
...
- **완료 (C-1 ~ Phase D)**:
  ...
  - Phase S: **한국 시군구 색칠 (조사 완료 2026-07-31, 진행 중)**. 세계는 나라 단위 유지, **한국만 시군구 단위로 세분**. 지도 색칠은 진입점이라는 제품 정의는 그대로.
    ...
    - **경계 데이터 — 1순위/2순위 (상세 비교는 조사 기록 참고)**:
      - **✅ 확정(S-0 완료, 2026-07-31): 공공데이터포털 `15125064` 국토교통부_(센서스경계)시군구경계**. 사용자가 브라우저로 데이터셋 페이지를 직접 실측해 확정한 값:
        - **이용허락범위: `제한 없음`** → **상업적 이용·변형 모두 가능**(단순화·좌표변환·DB적재 전부 허용). 아래 "외부 데이터 라이선스 원칙"의 통과 조건 충족.
        - 제공기관 **국토교통부** / 관리부서 **국가공간정보센터** / 비용 **무료** / 확장자 **SHP** / 공간범위 **대한민국 전체**
        - 수정일 **2025-07-01**, 업데이트 주기 **연간**, **차기 등록 예정일 2026-07-01** → 갱신 시점이 예측 가능하다(행정구역 개편 반영도 이 주기를 따라간다고 보면 된다).
        - 제공형태: 기관자체 다운로드 → `https://www.vworld.kr/dtmk/dtmk_ntads_s002.do?dsId=30015`
        - 스크린샷 증거는 `docs/licenses/`에 보관(라이선스 원칙 섹션의 보관 의무).
      - ⚠️ **미해결: vworld가 502로 막혀 파일을 아직 못 받았다** (도구·브라우저 양쪽 모두). 라이선스는 확정됐으므로 **설계는 진행 가능**하고, 막힌 건 다운로드뿐이다. 계속 안 되면 2순위로 전환.
      - ⚠️⚠️ **전체 행 250 ≠ 시군구 229 — S-2 착수 시 가장 먼저 확인할 것.** 데이터셋 페이지 기준 전체 행이 250이라 표준 시군구 개수(229)와 21행 차이가 난다. **센서스경계는 통계 집계용이라 자치단체 경계와 1:1이 아닐 수 있다** — 세종·제주 행정시 처리나 통계용 분할이 섞였을 가능성. **속성 테이블을 열어 행 정의부터 확인한 뒤 식별자 매핑을 설계할 것.** 229를 전제로 스키마를 먼저 짜지 말 것.
      - **2순위(안전판): `15059910` 국토지리정보원_기본공간정보** — `공공저작물 : 출처표시 (제1유형)`, SHP, 2025-06-23. ⚠️ **전용 S/W + 영역 신청 절차가 있어 받는 데 시간이 걸린다** — 1순위가 막힐 때만 간다.
    ...
    - **⭐ 시군구 식별자 — 코드 단독은 안전하지 않다 (확정)**: 행정구역 개편 시 **5자리 코드 자체가 바뀐다** — 군위군 대구 편입(2023-07-01)으로 `47720`(경북) → `27720`(대구). 앞 2자리가 시도라 편입되면 따라 바뀐다. 이름도 불가(중구·동구·남구 등 여러 시도에 중복). → **자체 surrogate id를 PK로 두고 `sgg_code`(5자리)·이름은 속성 컬럼으로.** 방문 기록은 surrogate를 참조하므로 코드가 바뀌어도 기록이 안 깨지고, 개편 시 매핑 테이블만 갱신하면 된다(229개 규모라 `valid_from`/`valid_to` 이력도 부담 없음). **코드를 자연키로 쓰면 편입 때마다 방문 기록 FK를 손대야 한다.**
    ...
    - **단계 분할**: **S-0 라이선스 확정 ✅ 완료**(소스 확정 / 다운로드·용량 측정은 vworld 502로 미완, S-2로 이월) → **S-1 city 잔재 정리 ✅ 완료**(커밋 `1f17991`) → S-2(**행 정의 확인 250 vs 229** + 경계 테이블+적재) → S-3(`posts.sgg_id`+판정 트리거+backfill) → S-4(방문 테이블+RLS) → S-5(지도 렌더링) → S-6(시군구 상세/색 선택). **출처표시 UI("지도 데이터 출처")는 S-5 또는 S-6 범위에 포함**(아래 라이선스 원칙 참고).
...
- **⚠️ 포털 메타데이터의 라이선스와 실제 배포처 약관이 다를 수 있다.** 같은 계열이어도 공공데이터포털은 "제한 없음"인데 국가공간정보포털 오픈마켓은 CC BY-NC-ND인 사례가 실제로 있었다(Phase S). **다운로드가 외부 사이트로 넘어가면 그쪽 약관도 확인할 것.**
