...
- **⭐ 다음 세션 시작점**: 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  ...
  2. ✅ **좋아요 — 완료 (Phase P)**
  3. ✅ **댓글 — 완료 (Phase Q)**
  4. ⭐ **진행 중: 한국 시군구 색칠 (Phase S)** — **S-0~S-3 완료 (2026-08-16)**. 소스는 OSM/ODbL로 재확정했고, 경계 230개 확보·적재·서버측 판정 트리거까지 끝났다. **다음은 S-4(시군구 방문 테이블 + RLS)**.
     - **S-4 착수 시 반드시 지킬 것**: SELECT 정책은 **`can_view_post` 경유**(조건 복붙 금지), INSERT/UPDATE도 `country_visits_insert_requires_post`와 같은 exists 조건, **생성·삭제는 트리거 전담·앱은 색만 UPDATE**(G-1/G-2 원칙 상속). 인덱스는 이미 만들어둔 `posts_sgg_idx(sgg_id, user_id)`가 그 서브쿼리에 맞는다.
     ...
     - ⚠️ **미결 3 — `git push`**: 경계 파일이 로컬 커밋만 된 상태면 **ODbL 4.6이 충족되지 않는다**(공개돼야 효력). 앱 배포 전까지는 반드시 푸시할 것.
  5. 장소검색(지오코딩)
...
- **완료 (C-1 ~ Phase D)**:
  ...
  - Phase S: **한국 시군구 색칠 (조사 완료 2026-07-31, 진행 중)**. 세계는 나라 단위 유지, **한국만 시군구 단위로 세분**. 지도 색칠은 진입점이라는 제품 정의는 그대로.
    ...
    - **S-3 완료 (2026-08-16, 마이그레이션 `20260816100000_sgg_boundaries.sql`)** — 경계 테이블 + `posts.sgg_id` + 서버측 판정 트리거.
      ...
      - **관찰 노트(조치 안 함)**: Nominatim이 구례군의 상위를 **"전남광주통합특별시"**로 표기한다 — 전남·광주 통합이 OSM에 반영돼 있다는 뜻. `admin_level=4` 변화라 시군구 레이어에는 영향이 없지만, **통합되면 시군구 코드 앞 2자리가 바뀐다**(군위군 사례와 동일). surrogate id 설계가 정확히 이 경우를 위한 것이므로 기록만 해둔다.
