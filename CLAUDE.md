...
- **⭐ 다음 세션 시작점**: 아래 로드맵 순서대로 (배경은 바로 아래 "⭐ 출시 시점 방향 결정" 참고).
  ...
  2. ✅ **좋아요 — 완료 (Phase P)**
  3. ✅ **댓글 — 완료 (Phase Q)**
  4. ⭐ **다음: 한국 시군구 색칠 (Phase S — 방향 확정, 조사 완료)** / 장소검색(지오코딩)
...
- **완료 (C-1 ~ Phase D)**:
  ...
  - Phase S: **한국 시군구 색칠 (조사 완료 2026-07-31, 진행 중)**. 세계는 나라 단위 유지, **한국만 시군구(약 229개)로 세분**. 지도 색칠은 진입점이라는 제품 정의는 그대로.
    ...
    - **경계 데이터 — 1순위/2순위 (상세 비교는 조사 기록 참고)**:
      - **1순위: 공공데이터포털 `15125064` 국토교통부_(센서스경계)시군구경계** — `이용허락범위 제한 없음`, SHP, 무료·무가입, 수정일 2025-07-01("연단위 갱신"). ⚠️ **실제 다운로드는 vworld.kr(dsId=30015)에서 일어나고 vworld 약관은 미확인** — 자동 접근이 막혀 있어(curl `http=000`, WebFetch 502) **사용자가 브라우저로 직접 확인**하기로 함(S-0).
    ...
    - **단계 분할**: S-0(약관 확인·소스 확정·용량 측정) → **S-1 city 잔재 정리 ✅ 완료** → S-2(경계 테이블+적재) → S-3(`posts.sgg_id`+판정 트리거+backfill) → S-4(방문 테이블+RLS) → S-5(지도 렌더링) → S-6(시군구 상세/색 선택). **출처표시 UI("지도 데이터 출처")는 S-5 또는 S-6 범위에 포함**(아래 라이선스 원칙 참고).
