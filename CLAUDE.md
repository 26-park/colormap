  ...
  5. ⭐⭐ **진행 중: 출시 전 실기기 다듬기 (Phase T) — 2026-08-23 추가**. 갤럭시 실기기에 최신 빌드를 올려 **실제로 써보면서 다듬는다.** 새 기능을 넓히는 단계가 아니라, 에뮬에서 안 보이던 것(터치 타깃, 실제 GPS, 실제 사진, 스크롤 감, 성능, 권한 다이얼로그)을 잡는 단계다.
     ...
     - **⭐⭐ Phase T 남은 목록 (2026-08-31 3차 실기기 검증 후 확정 — 다음 세션은 여기서 시작)**:
       ...
       - **✅ [T-2] GPS 없이 수동으로 핀을 찍을 수 있어야 한다 — 구현·에뮬 검증 완료 (2026-09-10). 실기기 잔여 3건**: "현재 위치"를 눌러도 **핀이 안 찍히고 게시도 영영 안 열리는** 경우가 있었다(실기기).
         ...
         - **✅ 원래도 정상이었던 것(고치지 않음)**: 권한 거부 경로는 이미 안내 문구 + `gpsLoading` 해제로 **수동 핀 진행이 가능**했다. 미니맵 탭(`onPress`)·핀 드래그(`ViewAnnotation draggable`)도 GPS와 무관하게 상시 동작하고, "지도를 탭하거나 핀을 끌어 옮기세요" 안내도 **상시 노출**돼 있었다. **즉 수동 핀 기능 자체는 이미 완성돼 있었고, 유일하게 막던 게 `!gpsLoading`이었다.**
         ...
         - **⛔ 실기기 잔여 3건 (에뮬에서 원리적으로 확인 불가)**:
           - ① **핀 시각 확인 + 드래그** — 에뮬은 `ViewAnnotation`을 아예 렌더하지 않는다(A/B로 원본에서도 동일함을 확인).
...
- ⭐ **MapLibre `ViewAnnotation`이 에뮬에서 렌더되지 않는다 (핀이 아예 안 보인다).** 작성 화면에서 지도를 탭하면 `pickedCoord`는 정상으로 잡혀 나라명·게시 버튼이 살아나는데 **핀만 안 그려진다.** 실기기에서는 2순위(`65f1db5`)에서 드래그까지 검증된 기능이다.
  - ⚠️ **핀이 안 보이면 자기 변경을 의심하기 전에 A/B부터 할 것** — `git stash`로 직전 커밋 상태에 돌려 같은 조작을 재현해보면 5분 안에 갈린다. 실제로 이 방법으로 "원본에서도 안 보인다"를 확인하고 오진을 피했다.
