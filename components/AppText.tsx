...
// 시스템 글꼴 크기 배율의 상한.
// 사용자가 글자 크기를 최대로 올리면 배율이 그대로 곱해져 고정 높이 UI(헤더,
// 칩, 버튼, 통계 카드)가 통째로 깨진다 — 실기기 보고이고 에뮬레이터
// font_scale 1.5로 재현했다. 접근성을 위해 확대 자체는 따르되 상한을 둔다.
// 이 파일이 앱 전체 Text의 단일 통로라(18개 파일 전부 여기서 import) 여기
// 한 곳만 막으면 전역에 적용된다.
// ⚠️ 개별 화면에서 더 낮은 상한이 필요하면 그 Text에 직접 주면 덮어쓴다
//    (하단 탭 라벨이 1.2로 그렇게 하고 있다).
