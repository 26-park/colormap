...
// 정확히 교대시킨다 — 둘이 겹쳐 칠해지면 색이 섞이고, 해안선이 어긋나
// 아래 나라색이 테두리로 샌다(4순위 조사에서 실측, CLAUDE.md 참고).
//
// ⭐ 6 → 4 (2026-09-10, Phase T 4순위): 줌 4~6 구간에서 한국이 화면에 크게
// 보이는데도 한 덩어리라 시군구 색칠이 안 보이던 문제를 해소한다. 4가 하한선인
// 근거는 시군구 하나가 약 9.6dp가 되는 지점이라는 것(z3은 4.8dp로 점 수준).
// 이 값을 더 내려 "항상 켜기"로 가면 안 되는 이유는 CLAUDE.md에 정리해뒀다.
const SGG_MIN_ZOOM = 4;
